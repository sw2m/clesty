/**
 * Red-Gate scaffold for sw2m/clesty issue #169 — Parameter `style: spaceDelimited`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #169 grows a tech
...
// grows the wiring for Parameter `style: spaceDelimited`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-style-spacedelimited/", import.meta.url),
);
...
Deno.test.ignore(
  "#169 (wip): Parameter `style: spaceDelimited` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #169 pins down the
...
Deno.test.ignore(
  "#169 (wip): Parameter `style: spaceDelimited` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #169 pins the
