/**
 * Red-Gate scaffold for sw2m/clesty issue #386 — Schema `default` interacts with `required`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #386 grows a tech
...
// grows the wiring for Schema `default` interacts with `required`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-default-interacts-with-required/", import.meta.url),
);
...
Deno.test.ignore(
  "#386 (wip): Schema `default` interacts with `required` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #386 pins down the
...
Deno.test.ignore(
  "#386 (wip): Schema `default` interacts with `required` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #386 pins the
