/**
 * Red-Gate scaffold for sw2m/clesty issue #367 — Trailing slash semantics.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #367 grows a tech
...
// grows the wiring for Trailing slash semantics.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/trailing-slash-semantics/", import.meta.url),
);
...
Deno.test.ignore(
  "#367 (wip): Trailing slash semantics — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #367 pins down the
...
Deno.test.ignore(
  "#367 (wip): Trailing slash semantics — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #367 pins the
