/**
 * Red-Gate scaffold for sw2m/clesty issue #345 — `xml.wrapped`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #345 grows a tech
...
// grows the wiring for `xml.wrapped`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/xml-wrapped/", import.meta.url),
);
...
Deno.test.ignore(
  "#345 (wip): `xml.wrapped` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #345 pins down the
...
Deno.test.ignore(
  "#345 (wip): `xml.wrapped` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #345 pins the
