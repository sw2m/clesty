/**
 * Red-Gate scaffold for sw2m/clesty issue #351 — Discriminator with `oneOf`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #351 grows a tech
...
// grows the wiring for Discriminator with `oneOf`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/discriminator-with-oneof/", import.meta.url),
);
...
Deno.test.ignore(
  "#351 (wip): Discriminator with `oneOf` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #351 pins down the
...
Deno.test.ignore(
  "#351 (wip): Discriminator with `oneOf` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #351 pins the
