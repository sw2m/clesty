/**
 * Red-Gate scaffold for sw2m/clesty issue #422 — `info.version` semver-like vs free-form.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #422 grows a tech
...
// grows the wiring for `info.version` semver-like vs free-form.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/info-version-semver-like-vs-free-form/", import.meta.url),
);
...
Deno.test.ignore(
  "#422 (wip): `info.version` semver-like vs free-form — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #422 pins down the
...
Deno.test.ignore(
  "#422 (wip): `info.version` semver-like vs free-form — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #422 pins the
