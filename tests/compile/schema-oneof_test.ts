/**
 * Red-Gate scaffold for sw2m/clesty issue #223 — Schema `oneOf`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #223 grows a tech
...
// grows the wiring for Schema `oneOf`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-oneof/", import.meta.url),
);
...
Deno.test.ignore(
  "#223 (wip): Schema `oneOf` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #223 pins down the
...
Deno.test.ignore(
  "#223 (wip): Schema `oneOf` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #223 pins the
