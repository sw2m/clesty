/**
 * Red-Gate scaffold for sw2m/clesty issue #373 — Doc-level → operation-level `security` inheritance.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #373 grows a tech
...
// grows the wiring for Doc-level → operation-level `security` inheritance.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/doc-level-operation-level-security-inheritance/", import.meta.url),
);
...
Deno.test.ignore(
  "#373 (wip): Doc-level → operation-level `security` inheritance — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #373 pins down the
...
Deno.test.ignore(
  "#373 (wip): Doc-level → operation-level `security` inheritance — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #373 pins the
