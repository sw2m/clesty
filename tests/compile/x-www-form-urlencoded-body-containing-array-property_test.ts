/**
 * Red-Gate scaffold for sw2m/clesty issue #412 — `x-www-form-urlencoded` body containing array property.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #412 grows a tech
...
// grows the wiring for `x-www-form-urlencoded` body containing array property.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/x-www-form-urlencoded-body-containing-array-property/", import.meta.url),
);
...
Deno.test.ignore(
  "#412 (wip): `x-www-form-urlencoded` body containing array property — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #412 pins down the
...
Deno.test.ignore(
  "#412 (wip): `x-www-form-urlencoded` body containing array property — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #412 pins the
