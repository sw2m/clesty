/**
 * Red-Gate scaffold for sw2m/clesty issue #342 — `xml.prefix`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #342 grows a tech
...
// grows the wiring for `xml.prefix`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/xml-prefix/", import.meta.url),
);
...
Deno.test.ignore(
  "#342 (wip): `xml.prefix` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #342 pins down the
...
Deno.test.ignore(
  "#342 (wip): `xml.prefix` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #342 pins the
