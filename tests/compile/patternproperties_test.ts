/**
 * Red-Gate scaffold for sw2m/clesty issue #253 — `patternProperties`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #253 grows a tech
...
// grows the wiring for `patternProperties`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/patternproperties/", import.meta.url),
);
...
Deno.test.ignore(
  "#253 (wip): `patternProperties` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #253 pins down the
...
Deno.test.ignore(
  "#253 (wip): `patternProperties` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #253 pins the
