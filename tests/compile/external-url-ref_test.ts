/**
 * Red-Gate scaffold for sw2m/clesty issue #279 — External URL `$ref`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #279 grows a tech
...
// grows the wiring for External URL `$ref`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/external-url-ref/", import.meta.url),
);
...
Deno.test.ignore(
  "#279 (wip): External URL `$ref` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #279 pins down the
...
Deno.test.ignore(
  "#279 (wip): External URL `$ref` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #279 pins the
