/**
 * Red-Gate scaffold for sw2m/clesty issue #389 — Tag declared on op but not in top-level `tags`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #389 grows a tech
...
// grows the wiring for Tag declared on op but not in top-level `tags`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/tag-declared-on-op-but-not-in-top-level-tags/", import.meta.url),
);
...
Deno.test.ignore(
  "#389 (wip): Tag declared on op but not in top-level `tags` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #389 pins down the
...
Deno.test.ignore(
  "#389 (wip): Tag declared on op but not in top-level `tags` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #389 pins the
