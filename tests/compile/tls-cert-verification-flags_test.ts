/**
 * Red-Gate scaffold for sw2m/clesty issue #450 — TLS / cert verification flags.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #450 grows a tech
...
// grows the wiring for TLS / cert verification flags.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/tls-cert-verification-flags/", import.meta.url),
);
...
Deno.test.ignore(
  "#450 (wip): TLS / cert verification flags — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #450 pins down the
...
Deno.test.ignore(
  "#450 (wip): TLS / cert verification flags — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #450 pins the
