/**
 * Red-Gate scaffold for sw2m/clesty issue #377 — Range `2XX` overlapping with exact `200`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #377 grows a tech
...
// grows the wiring for Range `2XX` overlapping with exact `200`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/range-2xx-overlapping-with-exact-200/", import.meta.url),
);
...
Deno.test.ignore(
  "#377 (wip): Range `2XX` overlapping with exact `200` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #377 pins down the
...
Deno.test.ignore(
  "#377 (wip): Range `2XX` overlapping with exact `200` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #377 pins the
