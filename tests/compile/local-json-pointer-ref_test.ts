/**
 * Red-Gate scaffold for sw2m/clesty issue #277 — Local JSON-pointer `$ref`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #277 grows a tech
...
// grows the wiring for Local JSON-pointer `$ref`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/local-json-pointer-ref/", import.meta.url),
);
...
Deno.test.ignore(
  "#277 (wip): Local JSON-pointer `$ref` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #277 pins down the
...
Deno.test.ignore(
  "#277 (wip): Local JSON-pointer `$ref` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #277 pins the
