/**
 * Red-Gate scaffold for sw2m/clesty issue #220 — Schema `properties`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #220 grows a tech
...
// grows the wiring for Schema `properties`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-properties/", import.meta.url),
);
...
Deno.test.ignore(
  "#220 (wip): Schema `properties` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #220 pins down the
...
Deno.test.ignore(
  "#220 (wip): Schema `properties` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #220 pins the
