/**
 * Red-Gate scaffold for sw2m/clesty issue #269 — `components.parameters`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #269 grows a tech
...
// grows the wiring for `components.parameters`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-parameters/", import.meta.url),
);
...
Deno.test.ignore(
  "#269 (wip): `components.parameters` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #269 pins down the
...
Deno.test.ignore(
  "#269 (wip): `components.parameters` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #269 pins the
