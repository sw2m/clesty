/**
 * Red-Gate scaffold for sw2m/clesty issue #257 — `$defs`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #257 grows a tech
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/defs/", import.meta.url),
);
...
Deno.test.ignore(
  "#257 (wip): `$defs` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #257 pins down the
...
Deno.test.ignore(
  "#257 (wip): `$defs` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #257 pins the
