/**
 * Red-Gate scaffold for sw2m/clesty issue #267 — `components.schemas`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #267 grows a tech
...
// grows the wiring for `components.schemas`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-schemas/", import.meta.url),
);
...
Deno.test.ignore(
  "#267 (wip): `components.schemas` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #267 pins down the
...
Deno.test.ignore(
  "#267 (wip): `components.schemas` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #267 pins the
