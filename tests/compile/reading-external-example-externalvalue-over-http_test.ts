/**
 * Red-Gate scaffold for sw2m/clesty issue #408 — Reading external example (`externalValue`) over HTTP.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #408 grows a tech
...
// grows the wiring for Reading external example (`externalValue`) over HTTP.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/reading-external-example-externalvalue-over-http/", import.meta.url),
);
...
Deno.test.ignore(
  "#408 (wip): Reading external example (`externalValue`) over HTTP — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #408 pins down the
...
Deno.test.ignore(
  "#408 (wip): Reading external example (`externalValue`) over HTTP — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #408 pins the
