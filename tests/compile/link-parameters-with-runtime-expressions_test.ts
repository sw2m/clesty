/**
 * Red-Gate scaffold for sw2m/clesty issue #321 — Link `parameters` (with runtime expressions).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #321 grows a tech
...
// grows the wiring for Link `parameters` (with runtime expressions).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/link-parameters-with-runtime-expressions/", import.meta.url),
);
...
Deno.test.ignore(
  "#321 (wip): Link `parameters` (with runtime expressions) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #321 pins down the
...
Deno.test.ignore(
  "#321 (wip): Link `parameters` (with runtime expressions) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #321 pins the
