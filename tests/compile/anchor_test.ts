/**
 * Red-Gate scaffold for sw2m/clesty issue #255 — `$anchor`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #255 grows a tech
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/anchor/", import.meta.url),
);
...
Deno.test.ignore(
  "#255 (wip): `$anchor` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #255 pins down the
...
Deno.test.ignore(
  "#255 (wip): `$anchor` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #255 pins the
