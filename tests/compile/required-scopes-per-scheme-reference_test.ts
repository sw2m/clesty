/**
 * Red-Gate scaffold for sw2m/clesty issue #304 — Required scopes per scheme reference.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #304 grows a tech
...
// grows the wiring for Required scopes per scheme reference.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/required-scopes-per-scheme-reference/", import.meta.url),
);
...
Deno.test.ignore(
  "#304 (wip): Required scopes per scheme reference — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #304 pins down the
...
Deno.test.ignore(
  "#304 (wip): Required scopes per scheme reference — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #304 pins the
