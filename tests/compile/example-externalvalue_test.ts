/**
 * Red-Gate scaffold for sw2m/clesty issue #333 — Example `externalValue`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #333 grows a tech
...
// grows the wiring for Example `externalValue`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/example-externalvalue/", import.meta.url),
);
...
Deno.test.ignore(
  "#333 (wip): Example `externalValue` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #333 pins down the
...
Deno.test.ignore(
  "#333 (wip): Example `externalValue` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #333 pins the
