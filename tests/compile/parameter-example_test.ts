/**
 * Red-Gate scaffold for sw2m/clesty issue #175 — Parameter `example`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #175 grows a tech
...
// grows the wiring for Parameter `example`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-example/", import.meta.url),
);
...
Deno.test.ignore(
  "#175 (wip): Parameter `example` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #175 pins down the
...
Deno.test.ignore(
  "#175 (wip): Parameter `example` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #175 pins the
