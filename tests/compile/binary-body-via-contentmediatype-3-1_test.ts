/**
 * Red-Gate scaffold for sw2m/clesty issue #194 — Binary body via `contentMediaType` (3.1).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #194 grows a tech
...
// grows the wiring for Binary body via `contentMediaType` (3.1).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/binary-body-via-contentmediatype-3-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#194 (wip): Binary body via `contentMediaType` (3.1) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #194 pins down the
...
Deno.test.ignore(
  "#194 (wip): Binary body via `contentMediaType` (3.1) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #194 pins the
