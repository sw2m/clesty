 ...
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #242 grows a tech
...
// grows the wiring for `type` as array (incl. `"null"`).
...
Deno.test.ignore(
  "#242 (wip): `type` as array (incl. `"null"`) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #242 pins down the
...
Deno.test.ignore(
  "#242 (wip): `type` as array (incl. `"null"`) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #242 pins the
