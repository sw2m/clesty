/**
 * Red-Gate scaffold for sw2m/clesty issue #426 — Doc with `paths` empty object.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #426 grows a tech
...
// grows the wiring for Doc with `paths` empty object.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/doc-with-paths-empty-object/", import.meta.url),
);
...
Deno.test.ignore(
  "#426 (wip): Doc with `paths` empty object — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #426 pins down the
...
Deno.test.ignore(
  "#426 (wip): Doc with `paths` empty object — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #426 pins the
