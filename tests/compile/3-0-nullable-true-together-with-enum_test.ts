/**
 * Red-Gate scaffold for sw2m/clesty issue #398 — 3.0 `nullable: true` together with `enum`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #398 grows a tech
...
// grows the wiring for 3.0 `nullable: true` together with `enum`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/3-0-nullable-true-together-with-enum/", import.meta.url),
);
...
Deno.test.ignore(
  "#398 (wip): 3.0 `nullable: true` together with `enum` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #398 pins down the
...
Deno.test.ignore(
  "#398 (wip): 3.0 `nullable: true` together with `enum` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #398 pins the
