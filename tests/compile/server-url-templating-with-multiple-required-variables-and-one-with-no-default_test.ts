/**
 * Red-Gate scaffold for sw2m/clesty issue #369 — Server URL templating with multiple required variables and one with no default.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #369 grows a tech
...
// grows the wiring for Server URL templating with multiple required variables and one with no default.
...
Deno.test.ignore(
  "#369 (wip): Server URL templating with multiple required variables and one with no default — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #369 pins down the
...
Deno.test.ignore(
  "#369 (wip): Server URL templating with multiple required variables and one with no default — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #369 pins the
