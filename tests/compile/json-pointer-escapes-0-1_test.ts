/**
 * Red-Gate scaffold for sw2m/clesty issue #357 — JSON Pointer escapes (`~0`/`~1`).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #357 grows a tech
...
// grows the wiring for JSON Pointer escapes (`~0`/`~1`).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/json-pointer-escapes-0-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#357 (wip): JSON Pointer escapes (`~0`/`~1`) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #357 pins down the
...
Deno.test.ignore(
  "#357 (wip): JSON Pointer escapes (`~0`/`~1`) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #357 pins the
