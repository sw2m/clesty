/**
 * Red-Gate scaffold for sw2m/clesty issue #451 — HTTP proxy honoring.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #451 grows a tech
...
// grows the wiring for HTTP proxy honoring.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/http-proxy-honoring/", import.meta.url),
);
...
Deno.test.ignore(
  "#451 (wip): HTTP proxy honoring — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #451 pins down the
...
Deno.test.ignore(
  "#451 (wip): HTTP proxy honoring — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #451 pins the
