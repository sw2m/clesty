/**
 * Red-Gate scaffold for sw2m/clesty issue #413 — JSON Schema `format` not in OAS table.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #413 grows a tech
...
// grows the wiring for JSON Schema `format` not in OAS table.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/json-schema-format-not-in-oas-table/", import.meta.url),
);
...
Deno.test.ignore(
  "#413 (wip): JSON Schema `format` not in OAS table — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #413 pins down the
...
Deno.test.ignore(
  "#413 (wip): JSON Schema `format` not in OAS table — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #413 pins the
