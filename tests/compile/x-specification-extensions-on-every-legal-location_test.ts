/**
 * Red-Gate scaffold for sw2m/clesty issue #346 — `x-*` specification extensions on every legal location.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #346 grows a tech
...
// grows the wiring for `x-*` specification extensions on every legal location.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/x-specification-extensions-on-every-legal-location/", import.meta.url),
);
...
Deno.test.ignore(
  "#346 (wip): `x-*` specification extensions on every legal location — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #346 pins down the
...
Deno.test.ignore(
  "#346 (wip): `x-*` specification extensions on every legal location — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #346 pins the
