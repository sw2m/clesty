/**
 * Red-Gate scaffold for sw2m/clesty issue #400 — OAuth scopes requested vs scopes available.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #400 grows a tech
...
// grows the wiring for OAuth scopes requested vs scopes available.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/oauth-scopes-requested-vs-scopes-available/", import.meta.url),
);
...
Deno.test.ignore(
  "#400 (wip): OAuth scopes requested vs scopes available — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #400 pins down the
...
Deno.test.ignore(
  "#400 (wip): OAuth scopes requested vs scopes available — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #400 pins the
