/**
 * Red-Gate scaffold for sw2m/clesty issue #319 — Link `operationRef`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #319 grows a tech
...
// grows the wiring for Link `operationRef`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/link-operationref/", import.meta.url),
);
...
Deno.test.ignore(
  "#319 (wip): Link `operationRef` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #319 pins down the
...
Deno.test.ignore(
  "#319 (wip): Link `operationRef` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #319 pins the
