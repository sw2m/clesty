/**
 * Red-Gate scaffold for sw2m/clesty issue #273 — `components.securitySchemes`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #273 grows a tech
...
// grows the wiring for `components.securitySchemes`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-securityschemes/", import.meta.url),
);
...
Deno.test.ignore(
  "#273 (wip): `components.securitySchemes` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #273 pins down the
...
Deno.test.ignore(
  "#273 (wip): `components.securitySchemes` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #273 pins the
