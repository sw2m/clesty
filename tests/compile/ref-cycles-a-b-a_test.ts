/**
 * Red-Gate scaffold for sw2m/clesty issue #355 — `$ref` cycles (A→B→A).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #355 grows a tech
...
// grows the wiring for `$ref` cycles (A→B→A).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ref-cycles-a-b-a/", import.meta.url),
);
...
Deno.test.ignore(
  "#355 (wip): `$ref` cycles (A→B→A) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #355 pins down the
...
Deno.test.ignore(
  "#355 (wip): `$ref` cycles (A→B→A) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #355 pins the
