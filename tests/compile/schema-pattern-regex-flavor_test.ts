/**
 * Red-Gate scaffold for sw2m/clesty issue #414 — Schema `pattern` regex flavor.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #414 grows a tech
...
// grows the wiring for Schema `pattern` regex flavor.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-pattern-regex-flavor/", import.meta.url),
);
...
Deno.test.ignore(
  "#414 (wip): Schema `pattern` regex flavor — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #414 pins down the
...
Deno.test.ignore(
  "#414 (wip): Schema `pattern` regex flavor — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #414 pins the
