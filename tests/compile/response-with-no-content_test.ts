/**
 * Red-Gate scaffold for sw2m/clesty issue #379 — Response with no `content`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #379 grows a tech
...
// grows the wiring for Response with no `content`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-with-no-content/", import.meta.url),
);
...
Deno.test.ignore(
  "#379 (wip): Response with no `content` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #379 pins down the
...
Deno.test.ignore(
  "#379 (wip): Response with no `content` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #379 pins the
