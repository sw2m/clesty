/**
 * Red-Gate scaffold for sw2m/clesty issue #447 — `--help` cycles.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #447 grows a tech
...
// grows the wiring for `--help` cycles.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/help-cycles/", import.meta.url),
);
...
Deno.test.ignore(
  "#447 (wip): `--help` cycles — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #447 pins down the
...
Deno.test.ignore(
  "#447 (wip): `--help` cycles — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #447 pins the
