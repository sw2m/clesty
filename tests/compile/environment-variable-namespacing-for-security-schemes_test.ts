/**
 * Red-Gate scaffold for sw2m/clesty issue #438 — Environment-variable namespacing for security schemes.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #438 grows a tech
...
// grows the wiring for Environment-variable namespacing for security schemes.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/environment-variable-namespacing-for-security-schemes/", import.meta.url),
);
...
Deno.test.ignore(
  "#438 (wip): Environment-variable namespacing for security schemes — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #438 pins down the
...
Deno.test.ignore(
  "#438 (wip): Environment-variable namespacing for security schemes — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #438 pins the
