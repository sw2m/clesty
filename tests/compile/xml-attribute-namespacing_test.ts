/**
 * Red-Gate scaffold for sw2m/clesty issue #344 — `xml.attribute` namespacing.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #344 grows a tech
...
// grows the wiring for `xml.attribute` namespacing.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/xml-attribute-namespacing/", import.meta.url),
);
...
Deno.test.ignore(
  "#344 (wip): `xml.attribute` namespacing — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #344 pins down the
...
Deno.test.ignore(
  "#344 (wip): `xml.attribute` namespacing — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #344 pins the
