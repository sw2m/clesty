/**
 * Red-Gate scaffold for sw2m/clesty issue #200 — Response `description`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #200 grows a tech
...
// grows the wiring for Response `description`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-description/", import.meta.url),
);
...
Deno.test.ignore(
  "#200 (wip): Response `description` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #200 pins down the
...
Deno.test.ignore(
  "#200 (wip): Response `description` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #200 pins the
