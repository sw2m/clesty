/**
 * Red-Gate scaffold for sw2m/clesty issue #409 — Encoding `headers` map containing a `$ref`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #409 grows a tech
...
// grows the wiring for Encoding `headers` map containing a `$ref`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/encoding-headers-map-containing-a-ref/", import.meta.url),
);
...
Deno.test.ignore(
  "#409 (wip): Encoding `headers` map containing a `$ref` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #409 pins down the
...
Deno.test.ignore(
  "#409 (wip): Encoding `headers` map containing a `$ref` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #409 pins the
