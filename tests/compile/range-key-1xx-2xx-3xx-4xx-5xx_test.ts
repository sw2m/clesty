/**
 * Red-Gate scaffold for sw2m/clesty issue #198 — Range key (`1XX`/`2XX`/`3XX`/`4XX`/`5XX`).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #198 grows a tech
...
// grows the wiring for Range key (`1XX`/`2XX`/`3XX`/`4XX`/`5XX`).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/range-key-1xx-2xx-3xx-4xx-5xx/", import.meta.url),
);
...
Deno.test.ignore(
  "#198 (wip): Range key (`1XX`/`2XX`/`3XX`/`4XX`/`5XX`) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #198 pins down the
...
Deno.test.ignore(
  "#198 (wip): Range key (`1XX`/`2XX`/`3XX`/`4XX`/`5XX`) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #198 pins the
