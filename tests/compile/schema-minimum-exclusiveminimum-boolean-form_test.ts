/**
 * Red-Gate scaffold for sw2m/clesty issue #213 — Schema `minimum` / `exclusiveMinimum` (boolean form).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #213 grows a tech
...
// grows the wiring for Schema `minimum` / `exclusiveMinimum` (boolean form).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-minimum-exclusiveminimum-boolean-form/", import.meta.url),
);
...
Deno.test.ignore(
  "#213 (wip): Schema `minimum` / `exclusiveMinimum` (boolean form) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #213 pins down the
...
Deno.test.ignore(
  "#213 (wip): Schema `minimum` / `exclusiveMinimum` (boolean form) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #213 pins the
