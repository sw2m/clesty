/**
 * Red-Gate scaffold for sw2m/clesty issue #335 — Exact media type match.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #335 grows a tech
...
// grows the wiring for Exact media type match.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/exact-media-type-match/", import.meta.url),
);
...
Deno.test.ignore(
  "#335 (wip): Exact media type match — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #335 pins down the
...
Deno.test.ignore(
  "#335 (wip): Exact media type match — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #335 pins the
