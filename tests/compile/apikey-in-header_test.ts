/**
 * Red-Gate scaffold for sw2m/clesty issue #286 — `apiKey` — `in: header`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #286 grows a tech
...
// grows the wiring for `apiKey` — `in: header`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/apikey-in-header/", import.meta.url),
);
...
Deno.test.ignore(
  "#286 (wip): `apiKey` — `in: header` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #286 pins down the
...
Deno.test.ignore(
  "#286 (wip): `apiKey` — `in: header` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #286 pins the
