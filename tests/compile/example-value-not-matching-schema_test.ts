/**
 * Red-Gate scaffold for sw2m/clesty issue #387 — `example` value not matching `schema`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #387 grows a tech
...
// grows the wiring for `example` value not matching `schema`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/example-value-not-matching-schema/", import.meta.url),
);
...
Deno.test.ignore(
  "#387 (wip): `example` value not matching `schema` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #387 pins down the
...
Deno.test.ignore(
  "#387 (wip): `example` value not matching `schema` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #387 pins the
