/**
 * Red-Gate scaffold for sw2m/clesty issue #199 — Range vs exact precedence.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #199 grows a tech
...
// grows the wiring for Range vs exact precedence.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/range-vs-exact-precedence/", import.meta.url),
);
...
Deno.test.ignore(
  "#199 (wip): Range vs exact precedence — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #199 pins down the
...
Deno.test.ignore(
  "#199 (wip): Range vs exact precedence — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #199 pins the
