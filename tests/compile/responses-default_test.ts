/**
 * Red-Gate scaffold for sw2m/clesty issue #196 — Responses `default`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #196 grows a tech
...
// grows the wiring for Responses `default`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/responses-default/", import.meta.url),
);
...
Deno.test.ignore(
  "#196 (wip): Responses `default` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #196 pins down the
...
Deno.test.ignore(
  "#196 (wip): Responses `default` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #196 pins the
