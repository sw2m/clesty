/**
 * Red-Gate scaffold for sw2m/clesty issue #314 — Runtime expression `$request.query.<name>`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #314 grows a tech
...
// grows the wiring for Runtime expression `$request.query.<name>`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-request-query-name/", import.meta.url),
);
...
Deno.test.ignore(
  "#314 (wip): Runtime expression `$request.query.<name>` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #314 pins down the
...
Deno.test.ignore(
  "#314 (wip): Runtime expression `$request.query.<name>` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #314 pins the
