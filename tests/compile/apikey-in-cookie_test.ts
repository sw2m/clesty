/**
 * Red-Gate scaffold for sw2m/clesty issue #287 — `apiKey` — `in: cookie`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #287 grows a tech
...
// grows the wiring for `apiKey` — `in: cookie`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/apikey-in-cookie/", import.meta.url),
);
...
Deno.test.ignore(
  "#287 (wip): `apiKey` — `in: cookie` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #287 pins down the
...
Deno.test.ignore(
  "#287 (wip): `apiKey` — `in: cookie` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #287 pins the
