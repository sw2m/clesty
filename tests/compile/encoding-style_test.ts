/**
 * Red-Gate scaffold for sw2m/clesty issue #187 — Encoding `style`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #187 grows a tech
...
// grows the wiring for Encoding `style`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/encoding-style/", import.meta.url),
);
...
Deno.test.ignore(
  "#187 (wip): Encoding `style` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #187 pins down the
...
Deno.test.ignore(
  "#187 (wip): Encoding `style` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #187 pins the
