/**
 * Red-Gate scaffold for sw2m/clesty issue #293 — `oauth2.clientCredentials` flow.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #293 grows a tech
...
// grows the wiring for `oauth2.clientCredentials` flow.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/oauth2-clientcredentials-flow/", import.meta.url),
);
...
Deno.test.ignore(
  "#293 (wip): `oauth2.clientCredentials` flow — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #293 pins down the
...
Deno.test.ignore(
  "#293 (wip): `oauth2.clientCredentials` flow — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #293 pins the
