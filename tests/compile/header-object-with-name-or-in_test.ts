/**
 * Red-Gate scaffold for sw2m/clesty issue #410 — Header object with `name` or `in`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #410 grows a tech
...
// grows the wiring for Header object with `name` or `in`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/header-object-with-name-or-in/", import.meta.url),
);
...
Deno.test.ignore(
  "#410 (wip): Header object with `name` or `in` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #410 pins down the
...
Deno.test.ignore(
  "#410 (wip): Header object with `name` or `in` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #410 pins the
