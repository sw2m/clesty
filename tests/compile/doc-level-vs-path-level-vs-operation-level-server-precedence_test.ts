/**
 * Red-Gate scaffold for sw2m/clesty issue #305 — Doc-level vs path-level vs operation-level server precedence.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #305 grows a tech
...
// grows the wiring for Doc-level vs path-level vs operation-level server precedence.
...
Deno.test.ignore(
  "#305 (wip): Doc-level vs path-level vs operation-level server precedence — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #305 pins down the
...
Deno.test.ignore(
  "#305 (wip): Doc-level vs path-level vs operation-level server precedence — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #305 pins the
