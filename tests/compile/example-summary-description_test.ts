/**
 * Red-Gate scaffold for sw2m/clesty issue #331 — Example `summary` / `description`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #331 grows a tech
...
// grows the wiring for Example `summary` / `description`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/example-summary-description/", import.meta.url),
);
...
Deno.test.ignore(
  "#331 (wip): Example `summary` / `description` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #331 pins down the
...
Deno.test.ignore(
  "#331 (wip): Example `summary` / `description` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #331 pins the
