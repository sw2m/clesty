/**
 * Red-Gate scaffold for sw2m/clesty issue #210 — Schema `enum`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #210 grows a tech
...
// grows the wiring for Schema `enum`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-enum/", import.meta.url),
);
...
Deno.test.ignore(
  "#210 (wip): Schema `enum` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #210 pins down the
...
Deno.test.ignore(
  "#210 (wip): Schema `enum` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #210 pins the
