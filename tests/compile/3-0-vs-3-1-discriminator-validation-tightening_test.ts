/**
 * Red-Gate scaffold for sw2m/clesty issue #354 — 3.0 vs 3.1 discriminator validation tightening.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #354 grows a tech
...
// grows the wiring for 3.0 vs 3.1 discriminator validation tightening.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/3-0-vs-3-1-discriminator-validation-tightening/", import.meta.url),
);
...
Deno.test.ignore(
  "#354 (wip): 3.0 vs 3.1 discriminator validation tightening — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #354 pins down the
...
Deno.test.ignore(
  "#354 (wip): 3.0 vs 3.1 discriminator validation tightening — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #354 pins the
