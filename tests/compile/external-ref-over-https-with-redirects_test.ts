/**
 * Red-Gate scaffold for sw2m/clesty issue #360 — External `$ref` over HTTPS with redirects.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #360 grows a tech
...
// grows the wiring for External `$ref` over HTTPS with redirects.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/external-ref-over-https-with-redirects/", import.meta.url),
);
...
Deno.test.ignore(
  "#360 (wip): External `$ref` over HTTPS with redirects — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #360 pins down the
...
Deno.test.ignore(
  "#360 (wip): External `$ref` over HTTPS with redirects — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #360 pins the
