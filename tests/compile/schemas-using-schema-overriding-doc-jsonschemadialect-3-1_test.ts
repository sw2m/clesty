/**
 * Red-Gate scaffold for sw2m/clesty issue #415 — Schemas using `$schema` overriding doc `jsonSchemaDialect` (3.1).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #415 grows a tech
...
// grows the wiring for Schemas using `$schema` overriding doc `jsonSchemaDialect` (3.1).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schemas-using-schema-overriding-doc-jsonschemadialect-3-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#415 (wip): Schemas using `$schema` overriding doc `jsonSchemaDialect` (3.1) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #415 pins down the
...
Deno.test.ignore(
  "#415 (wip): Schemas using `$schema` overriding doc `jsonSchemaDialect` (3.1) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #415 pins the
