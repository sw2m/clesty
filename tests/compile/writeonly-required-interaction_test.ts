/**
 * Red-Gate scaffold for sw2m/clesty issue #230 — `writeOnly` + `required` interaction.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #230 grows a tech
...
// grows the wiring for `writeOnly` + `required` interaction.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/writeonly-required-interaction/", import.meta.url),
);
...
Deno.test.ignore(
  "#230 (wip): `writeOnly` + `required` interaction — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #230 pins down the
...
Deno.test.ignore(
  "#230 (wip): `writeOnly` + `required` interaction — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #230 pins the
