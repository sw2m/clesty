/**
 * Red-Gate scaffold for sw2m/clesty issue #417 — Components reachable only via `$ref` chain through `webhooks`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #417 grows a tech
...
// grows the wiring for Components reachable only via `$ref` chain through `webhooks`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-reachable-only-via-ref-chain-through-webhooks/", import.meta.url),
);
...
Deno.test.ignore(
  "#417 (wip): Components reachable only via `$ref` chain through `webhooks` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #417 pins down the
...
Deno.test.ignore(
  "#417 (wip): Components reachable only via `$ref` chain through `webhooks` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #417 pins the
