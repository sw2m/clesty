/**
 * Red-Gate scaffold for sw2m/clesty issue #328 — Webhook vs callback distinction.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #328 grows a tech
...
// grows the wiring for Webhook vs callback distinction.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/webhook-vs-callback-distinction/", import.meta.url),
);
...
Deno.test.ignore(
  "#328 (wip): Webhook vs callback distinction — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #328 pins down the
...
Deno.test.ignore(
  "#328 (wip): Webhook vs callback distinction — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #328 pins the
