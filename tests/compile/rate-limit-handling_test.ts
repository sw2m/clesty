/**
 * Red-Gate scaffold for sw2m/clesty issue #440 — Rate limit handling.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #440 grows a tech
...
// grows the wiring for Rate limit handling.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/rate-limit-handling/", import.meta.url),
);
...
Deno.test.ignore(
  "#440 (wip): Rate limit handling — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #440 pins down the
...
Deno.test.ignore(
  "#440 (wip): Rate limit handling — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #440 pins the
