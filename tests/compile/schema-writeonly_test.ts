/**
 * Red-Gate scaffold for sw2m/clesty issue #229 — Schema `writeOnly`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #229 grows a tech
...
// grows the wiring for Schema `writeOnly`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-writeonly/", import.meta.url),
);
...
Deno.test.ignore(
  "#229 (wip): Schema `writeOnly` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #229 pins down the
...
Deno.test.ignore(
  "#229 (wip): Schema `writeOnly` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #229 pins the
