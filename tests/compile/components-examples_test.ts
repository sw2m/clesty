/**
 * Red-Gate scaffold for sw2m/clesty issue #270 — `components.examples`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #270 grows a tech
...
// grows the wiring for `components.examples`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-examples/", import.meta.url),
);
...
Deno.test.ignore(
  "#270 (wip): `components.examples` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #270 pins down the
...
Deno.test.ignore(
  "#270 (wip): `components.examples` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #270 pins the
