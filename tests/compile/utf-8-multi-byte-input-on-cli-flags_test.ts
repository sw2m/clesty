/**
 * Red-Gate scaffold for sw2m/clesty issue #420 — UTF-8 multi-byte input on CLI flags.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #420 grows a tech
...
// grows the wiring for UTF-8 multi-byte input on CLI flags.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/utf-8-multi-byte-input-on-cli-flags/", import.meta.url),
);
...
Deno.test.ignore(
  "#420 (wip): UTF-8 multi-byte input on CLI flags — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #420 pins down the
...
Deno.test.ignore(
  "#420 (wip): UTF-8 multi-byte input on CLI flags — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #420 pins the
