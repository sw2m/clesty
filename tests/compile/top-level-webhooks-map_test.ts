/**
 * Red-Gate scaffold for sw2m/clesty issue #326 — Top-level `webhooks` map.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #326 grows a tech
...
// grows the wiring for Top-level `webhooks` map.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/top-level-webhooks-map/", import.meta.url),
);
...
Deno.test.ignore(
  "#326 (wip): Top-level `webhooks` map — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #326 pins down the
...
Deno.test.ignore(
  "#326 (wip): Top-level `webhooks` map — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #326 pins the
