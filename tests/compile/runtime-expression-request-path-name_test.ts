/**
 * Red-Gate scaffold for sw2m/clesty issue #313 — Runtime expression `$request.path.<name>`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #313 grows a tech
...
// grows the wiring for Runtime expression `$request.path.<name>`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-request-path-name/", import.meta.url),
);
...
Deno.test.ignore(
  "#313 (wip): Runtime expression `$request.path.<name>` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #313 pins down the
...
Deno.test.ignore(
  "#313 (wip): Runtime expression `$request.path.<name>` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #313 pins the
