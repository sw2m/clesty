/**
 * Red-Gate scaffold for sw2m/clesty issue #359 — `$ref` to non-existent target.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #359 grows a tech
...
// grows the wiring for `$ref` to non-existent target.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ref-to-non-existent-target/", import.meta.url),
);
...
Deno.test.ignore(
  "#359 (wip): `$ref` to non-existent target — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #359 pins down the
...
Deno.test.ignore(
  "#359 (wip): `$ref` to non-existent target — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #359 pins the
