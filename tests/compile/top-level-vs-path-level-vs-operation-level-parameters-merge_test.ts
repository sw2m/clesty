/**
 * Red-Gate scaffold for sw2m/clesty issue #424 — Top-level vs path-level vs operation-level `parameters` merge.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #424 grows a tech
...
// grows the wiring for Top-level vs path-level vs operation-level `parameters` merge.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/top-level-vs-path-level-vs-operation-level-parameters-merge/", import.meta.url),
);
...
Deno.test.ignore(
  "#424 (wip): Top-level vs path-level vs operation-level `parameters` merge — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #424 pins down the
...
Deno.test.ignore(
  "#424 (wip): Top-level vs path-level vs operation-level `parameters` merge — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #424 pins the
