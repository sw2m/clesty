/**
 * Red-Gate scaffold for sw2m/clesty issue #437 — stdin ingestion for request bodies.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #437 grows a tech
...
// grows the wiring for stdin ingestion for request bodies.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/stdin-ingestion-for-request-bodies/", import.meta.url),
);
...
Deno.test.ignore(
  "#437 (wip): stdin ingestion for request bodies — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #437 pins down the
...
Deno.test.ignore(
  "#437 (wip): stdin ingestion for request bodies — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #437 pins the
