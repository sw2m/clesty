/**
 * Red-Gate scaffold for sw2m/clesty issue #238 — Schema `default` validates against schema.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #238 grows a tech
...
// grows the wiring for Schema `default` validates against schema.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-default-validates-against-schema/", import.meta.url),
);
...
Deno.test.ignore(
  "#238 (wip): Schema `default` validates against schema — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #238 pins down the
...
Deno.test.ignore(
  "#238 (wip): Schema `default` validates against schema — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #238 pins the
