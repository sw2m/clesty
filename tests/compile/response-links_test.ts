/**
 * Red-Gate scaffold for sw2m/clesty issue #207 — Response `links`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #207 grows a tech
...
// grows the wiring for Response `links`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-links/", import.meta.url),
);
...
Deno.test.ignore(
  "#207 (wip): Response `links` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #207 pins down the
...
Deno.test.ignore(
  "#207 (wip): Response `links` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #207 pins the
