/**
 * Red-Gate scaffold for sw2m/clesty issue #179 — `requestBody.description`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #179 grows a tech
...
// grows the wiring for `requestBody.description`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/requestbody-description/", import.meta.url),
);
...
Deno.test.ignore(
  "#179 (wip): `requestBody.description` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #179 pins down the
...
Deno.test.ignore(
  "#179 (wip): `requestBody.description` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #179 pins the
