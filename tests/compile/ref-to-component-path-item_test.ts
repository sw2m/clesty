/**
 * Red-Gate scaffold for sw2m/clesty issue #280 — `$ref` to component path item.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #280 grows a tech
...
// grows the wiring for `$ref` to component path item.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ref-to-component-path-item/", import.meta.url),
);
...
Deno.test.ignore(
  "#280 (wip): `$ref` to component path item — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #280 pins down the
...
Deno.test.ignore(
  "#280 (wip): `$ref` to component path item — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #280 pins the
