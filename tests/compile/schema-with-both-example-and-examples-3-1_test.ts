/**
 * Red-Gate scaffold for sw2m/clesty issue #397 — Schema with both `example` and `examples` (3.1).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #397 grows a tech
...
// grows the wiring for Schema with both `example` and `examples` (3.1).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-with-both-example-and-examples-3-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#397 (wip): Schema with both `example` and `examples` (3.1) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #397 pins down the
...
Deno.test.ignore(
  "#397 (wip): Schema with both `example` and `examples` (3.1) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #397 pins the
