/**
 * Red-Gate scaffold for sw2m/clesty issue #433 — `servers[].url` containing fragment or query.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #433 grows a tech
...
// grows the wiring for `servers[].url` containing fragment or query.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/servers-url-containing-fragment-or-query/", import.meta.url),
);
...
Deno.test.ignore(
  "#433 (wip): `servers[].url` containing fragment or query — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #433 pins down the
...
Deno.test.ignore(
  "#433 (wip): `servers[].url` containing fragment or query — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #433 pins the
