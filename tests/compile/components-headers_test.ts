/**
 * Red-Gate scaffold for sw2m/clesty issue #272 — `components.headers`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #272 grows a tech
...
// grows the wiring for `components.headers`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-headers/", import.meta.url),
);
...
Deno.test.ignore(
  "#272 (wip): `components.headers` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #272 pins down the
...
Deno.test.ignore(
  "#272 (wip): `components.headers` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #272 pins the
