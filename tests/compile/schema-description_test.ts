/**
 * Red-Gate scaffold for sw2m/clesty issue #240 — Schema `description`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #240 grows a tech
...
// grows the wiring for Schema `description`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-description/", import.meta.url),
);
...
Deno.test.ignore(
  "#240 (wip): Schema `description` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #240 pins down the
...
Deno.test.ignore(
  "#240 (wip): Schema `description` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #240 pins the
