/**
 * Red-Gate scaffold for sw2m/clesty issue #185 — Encoding `contentType`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #185 grows a tech
...
// grows the wiring for Encoding `contentType`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/encoding-contenttype/", import.meta.url),
);
...
Deno.test.ignore(
  "#185 (wip): Encoding `contentType` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #185 pins down the
...
Deno.test.ignore(
  "#185 (wip): Encoding `contentType` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #185 pins the
