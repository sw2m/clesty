/**
 * Red-Gate scaffold for sw2m/clesty issue #176 — Parameter `examples`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #176 grows a tech
...
// grows the wiring for Parameter `examples`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-examples/", import.meta.url),
);
...
Deno.test.ignore(
  "#176 (wip): Parameter `examples` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #176 pins down the
...
Deno.test.ignore(
  "#176 (wip): Parameter `examples` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #176 pins the
