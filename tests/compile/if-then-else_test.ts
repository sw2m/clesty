/**
 * Red-Gate scaffold for sw2m/clesty issue #245 — `if` / `then` / `else`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #245 grows a tech
...
// grows the wiring for `if` / `then` / `else`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/if-then-else/", import.meta.url),
);
...
Deno.test.ignore(
  "#245 (wip): `if` / `then` / `else` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #245 pins down the
...
Deno.test.ignore(
  "#245 (wip): `if` / `then` / `else` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #245 pins the
