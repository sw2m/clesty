/**
 * Red-Gate scaffold for sw2m/clesty issue #421 — Locale-independent number parsing.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #421 grows a tech
...
// grows the wiring for Locale-independent number parsing.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/locale-independent-number-parsing/", import.meta.url),
);
...
Deno.test.ignore(
  "#421 (wip): Locale-independent number parsing — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #421 pins down the
...
Deno.test.ignore(
  "#421 (wip): Locale-independent number parsing — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #421 pins the
