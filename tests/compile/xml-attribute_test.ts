/**
 * Red-Gate scaffold for sw2m/clesty issue #343 — `xml.attribute`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #343 grows a tech
...
// grows the wiring for `xml.attribute`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/xml-attribute/", import.meta.url),
);
...
Deno.test.ignore(
  "#343 (wip): `xml.attribute` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #343 pins down the
...
Deno.test.ignore(
  "#343 (wip): `xml.attribute` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #343 pins the
