/**
 * Red-Gate scaffold for sw2m/clesty issue #383 — Large enum cardinality (10,000+ entries).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #383 grows a tech
...
// grows the wiring for Large enum cardinality (10,000+ entries).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/large-enum-cardinality-10-000-entries/", import.meta.url),
);
...
Deno.test.ignore(
  "#383 (wip): Large enum cardinality (10,000+ entries) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #383 pins down the
...
Deno.test.ignore(
  "#383 (wip): Large enum cardinality (10,000+ entries) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #383 pins the
