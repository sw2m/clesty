/**
 * Red-Gate scaffold for sw2m/clesty issue #262 — `contentEncoding`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #262 grows a tech
...
// grows the wiring for `contentEncoding`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/contentencoding/", import.meta.url),
);
...
Deno.test.ignore(
  "#262 (wip): `contentEncoding` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #262 pins down the
...
Deno.test.ignore(
  "#262 (wip): `contentEncoding` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #262 pins the
