/**
 * Red-Gate scaffold for sw2m/clesty issue #276 — `components.pathItems`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #276 grows a tech
...
// grows the wiring for `components.pathItems`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-pathitems/", import.meta.url),
);
...
Deno.test.ignore(
  "#276 (wip): `components.pathItems` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #276 pins down the
...
Deno.test.ignore(
  "#276 (wip): `components.pathItems` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #276 pins the
