/**
 * Red-Gate scaffold for sw2m/clesty issue #300 — Security Requirement — empty array `[]`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #300 grows a tech
...
// grows the wiring for Security Requirement — empty array `[]`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/security-requirement-empty-array/", import.meta.url),
);
...
Deno.test.ignore(
  "#300 (wip): Security Requirement — empty array `[]` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #300 pins down the
...
Deno.test.ignore(
  "#300 (wip): Security Requirement — empty array `[]` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #300 pins the
