/**
 * Red-Gate scaffold for sw2m/clesty issue #234 — Schema `externalDocs`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #234 grows a tech
...
// grows the wiring for Schema `externalDocs`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-externaldocs/", import.meta.url),
);
...
Deno.test.ignore(
  "#234 (wip): Schema `externalDocs` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #234 pins down the
...
Deno.test.ignore(
  "#234 (wip): Schema `externalDocs` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #234 pins the
