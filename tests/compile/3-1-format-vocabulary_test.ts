/**
 * Red-Gate scaffold for sw2m/clesty issue #265 — 3.1 format vocabulary.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #265 grows a tech
...
// grows the wiring for 3.1 format vocabulary.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/3-1-format-vocabulary/", import.meta.url),
);
...
Deno.test.ignore(
  "#265 (wip): 3.1 format vocabulary — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #265 pins down the
...
Deno.test.ignore(
  "#265 (wip): 3.1 format vocabulary — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #265 pins the
