/**
 * Red-Gate scaffold for sw2m/clesty issue #368 — Case sensitivity.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #368 grows a tech
...
// grows the wiring for Case sensitivity.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/case-sensitivity/", import.meta.url),
);
...
Deno.test.ignore(
  "#368 (wip): Case sensitivity — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #368 pins down the
...
Deno.test.ignore(
  "#368 (wip): Case sensitivity — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #368 pins the
