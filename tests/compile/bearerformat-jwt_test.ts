/**
 * Red-Gate scaffold for sw2m/clesty issue #402 — `bearerFormat: JWT`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #402 grows a tech
...
// grows the wiring for `bearerFormat: JWT`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/bearerformat-jwt/", import.meta.url),
);
...
Deno.test.ignore(
  "#402 (wip): `bearerFormat: JWT` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #402 pins down the
...
Deno.test.ignore(
  "#402 (wip): `bearerFormat: JWT` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #402 pins the
