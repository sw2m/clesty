/**
 * Red-Gate scaffold for sw2m/clesty issue #310 — Runtime expression `$url`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #310 grows a tech
...
// grows the wiring for Runtime expression `$url`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-url/", import.meta.url),
);
...
Deno.test.ignore(
  "#310 (wip): Runtime expression `$url` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #310 pins down the
...
Deno.test.ignore(
  "#310 (wip): Runtime expression `$url` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #310 pins the
