/**
 * Red-Gate scaffold for sw2m/clesty issue #216 — Schema `maxItems` / `minItems`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #216 grows a tech
...
// grows the wiring for Schema `maxItems` / `minItems`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-maxitems-minitems/", import.meta.url),
);
...
Deno.test.ignore(
  "#216 (wip): Schema `maxItems` / `minItems` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #216 pins down the
...
Deno.test.ignore(
  "#216 (wip): Schema `maxItems` / `minItems` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #216 pins the
