/**
 * Red-Gate scaffold for sw2m/clesty issue #325 — Link `operationRef` + `operationId` mutual exclusion.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #325 grows a tech
...
// grows the wiring for Link `operationRef` + `operationId` mutual exclusion.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/link-operationref-operationid-mutual-exclusion/", import.meta.url),
);
...
Deno.test.ignore(
  "#325 (wip): Link `operationRef` + `operationId` mutual exclusion — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #325 pins down the
...
Deno.test.ignore(
  "#325 (wip): Link `operationRef` + `operationId` mutual exclusion — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #325 pins the
