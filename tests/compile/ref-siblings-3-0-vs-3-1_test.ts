/**
 * Red-Gate scaffold for sw2m/clesty issue #284 — `$ref` siblings — 3.0 vs 3.1.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #284 grows a tech
...
// grows the wiring for `$ref` siblings — 3.0 vs 3.1.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ref-siblings-3-0-vs-3-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#284 (wip): `$ref` siblings — 3.0 vs 3.1 — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #284 pins down the
...
Deno.test.ignore(
  "#284 (wip): `$ref` siblings — 3.0 vs 3.1 — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #284 pins the
