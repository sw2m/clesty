/**
 * Red-Gate scaffold for sw2m/clesty issue #294 — `oauth2.authorizationCode` flow.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #294 grows a tech
...
// grows the wiring for `oauth2.authorizationCode` flow.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/oauth2-authorizationcode-flow/", import.meta.url),
);
...
Deno.test.ignore(
  "#294 (wip): `oauth2.authorizationCode` flow — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #294 pins down the
...
Deno.test.ignore(
  "#294 (wip): `oauth2.authorizationCode` flow — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #294 pins the
