/**
 * Red-Gate scaffold for sw2m/clesty issue #439 — Pagination support.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #439 grows a tech
...
// grows the wiring for Pagination support.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/pagination-support/", import.meta.url),
);
...
Deno.test.ignore(
  "#439 (wip): Pagination support — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #439 pins down the
...
Deno.test.ignore(
  "#439 (wip): Pagination support — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #439 pins the
