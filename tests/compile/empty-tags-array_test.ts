/**
 * Red-Gate scaffold for sw2m/clesty issue #388 — Empty `tags` array.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #388 grows a tech
...
// grows the wiring for Empty `tags` array.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/empty-tags-array/", import.meta.url),
);
...
Deno.test.ignore(
  "#388 (wip): Empty `tags` array — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #388 pins down the
...
Deno.test.ignore(
  "#388 (wip): Empty `tags` array — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #388 pins the
