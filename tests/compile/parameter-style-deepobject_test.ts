/**
 * Red-Gate scaffold for sw2m/clesty issue #171 — Parameter `style: deepObject`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #171 grows a tech
...
// grows the wiring for Parameter `style: deepObject`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-style-deepobject/", import.meta.url),
);
...
Deno.test.ignore(
  "#171 (wip): Parameter `style: deepObject` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #171 pins down the
...
Deno.test.ignore(
  "#171 (wip): Parameter `style: deepObject` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #171 pins the
