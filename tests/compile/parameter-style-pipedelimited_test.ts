/**
 * Red-Gate scaffold for sw2m/clesty issue #170 — Parameter `style: pipeDelimited`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #170 grows a tech
...
// grows the wiring for Parameter `style: pipeDelimited`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-style-pipedelimited/", import.meta.url),
);
...
Deno.test.ignore(
  "#170 (wip): Parameter `style: pipeDelimited` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #170 pins down the
...
Deno.test.ignore(
  "#170 (wip): Parameter `style: pipeDelimited` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #170 pins the
