 ...
 *
 * staging: tests are stubbed until #399 grows a tech spec and the
...
Deno.test.ignore(
  "#399 (wip): primary contract",
  () => {
    // staging: replace once #399 pins the contract.
    assert(true, "wip");
  },
);
...
Deno.test.ignore(
  "#399 (wip): refusal / edge case",
  () => {
    // staging: replace once #399 pins the refusal shape.
    assert(true, "wip");
  },
);
