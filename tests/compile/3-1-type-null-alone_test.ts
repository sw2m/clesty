/**
 * Red-Gate scaffold for sw2m/clesty issue #399 — 3.1 `type: "null"` alone.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #399 grows a tech
...
// grows the wiring for 3.1 `type: "null"` alone.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/3-1-type-null-alone/", import.meta.url),
);
...
Deno.test.ignore(
  "#399 (wip): 3.1 `type: "null"` alone — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #399 pins down the
...
Deno.test.ignore(
  "#399 (wip): 3.1 `type: "null"` alone — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #399 pins the
