/**
 * Red-Gate scaffold for sw2m/clesty issue #237 — Schema `default`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #237 grows a tech
...
// grows the wiring for Schema `default`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-default/", import.meta.url),
);
...
Deno.test.ignore(
  "#237 (wip): Schema `default` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #237 pins down the
...
Deno.test.ignore(
  "#237 (wip): Schema `default` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #237 pins the
