/**
 * Red-Gate scaffold for sw2m/clesty issue #165 — Parameter `style: matrix`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #165 grows a tech
...
// grows the wiring for Parameter `style: matrix`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-style-matrix/", import.meta.url),
);
...
Deno.test.ignore(
  "#165 (wip): Parameter `style: matrix` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #165 pins down the
...
Deno.test.ignore(
  "#165 (wip): Parameter `style: matrix` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #165 pins the
