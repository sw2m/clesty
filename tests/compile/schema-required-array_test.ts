/**
 * Red-Gate scaffold for sw2m/clesty issue #219 — Schema `required` (array).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #219 grows a tech
...
// grows the wiring for Schema `required` (array).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-required-array/", import.meta.url),
);
...
Deno.test.ignore(
  "#219 (wip): Schema `required` (array) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #219 pins down the
...
Deno.test.ignore(
  "#219 (wip): Schema `required` (array) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #219 pins the
