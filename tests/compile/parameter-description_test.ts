/**
 * Red-Gate scaffold for sw2m/clesty issue #161 — Parameter `description`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #161 grows a tech
...
// grows the wiring for Parameter `description`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-description/", import.meta.url),
);
...
Deno.test.ignore(
  "#161 (wip): Parameter `description` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #161 pins down the
...
Deno.test.ignore(
  "#161 (wip): Parameter `description` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #161 pins the
