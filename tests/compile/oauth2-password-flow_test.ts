/**
 * Red-Gate scaffold for sw2m/clesty issue #292 — `oauth2.password` flow.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #292 grows a tech
...
// grows the wiring for `oauth2.password` flow.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/oauth2-password-flow/", import.meta.url),
);
...
Deno.test.ignore(
  "#292 (wip): `oauth2.password` flow — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #292 pins down the
...
Deno.test.ignore(
  "#292 (wip): `oauth2.password` flow — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #292 pins the
