/**
 * Red-Gate scaffold for sw2m/clesty issue #385 — `additionalProperties: true` vs `false` vs schema.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #385 grows a tech
...
// grows the wiring for `additionalProperties: true` vs `false` vs schema.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/additionalproperties-true-vs-false-vs-schema/", import.meta.url),
);
...
Deno.test.ignore(
  "#385 (wip): `additionalProperties: true` vs `false` vs schema — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #385 pins down the
...
Deno.test.ignore(
  "#385 (wip): `additionalProperties: true` vs `false` vs schema — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #385 pins the
