/**
 * Red-Gate scaffold for sw2m/clesty issue #215 — Schema `pattern`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #215 grows a tech
...
// grows the wiring for Schema `pattern`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-pattern/", import.meta.url),
);
...
Deno.test.ignore(
  "#215 (wip): Schema `pattern` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #215 pins down the
...
Deno.test.ignore(
  "#215 (wip): Schema `pattern` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #215 pins the
