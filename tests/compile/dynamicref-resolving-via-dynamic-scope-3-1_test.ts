/**
 * Red-Gate scaffold for sw2m/clesty issue #416 — `$dynamicRef` resolving via dynamic scope (3.1).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #416 grows a tech
...
// grows the wiring for `$dynamicRef` resolving via dynamic scope (3.1).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/dynamicref-resolving-via-dynamic-scope-3-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#416 (wip): `$dynamicRef` resolving via dynamic scope (3.1) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #416 pins down the
...
Deno.test.ignore(
  "#416 (wip): `$dynamicRef` resolving via dynamic scope (3.1) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #416 pins the
