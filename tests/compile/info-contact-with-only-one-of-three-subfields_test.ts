/**
 * Red-Gate scaffold for sw2m/clesty issue #390 — `info.contact` with only one of three subfields.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #390 grows a tech
...
// grows the wiring for `info.contact` with only one of three subfields.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/info-contact-with-only-one-of-three-subfields/", import.meta.url),
);
...
Deno.test.ignore(
  "#390 (wip): `info.contact` with only one of three subfields — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #390 pins down the
...
Deno.test.ignore(
  "#390 (wip): `info.contact` with only one of three subfields — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #390 pins the
