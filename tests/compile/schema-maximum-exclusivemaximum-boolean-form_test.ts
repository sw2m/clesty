/**
 * Red-Gate scaffold for sw2m/clesty issue #212 — Schema `maximum` / `exclusiveMaximum` (boolean form).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #212 grows a tech
...
// grows the wiring for Schema `maximum` / `exclusiveMaximum` (boolean form).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-maximum-exclusivemaximum-boolean-form/", import.meta.url),
);
...
Deno.test.ignore(
  "#212 (wip): Schema `maximum` / `exclusiveMaximum` (boolean form) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #212 pins down the
...
Deno.test.ignore(
  "#212 (wip): Schema `maximum` / `exclusiveMaximum` (boolean form) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #212 pins the
