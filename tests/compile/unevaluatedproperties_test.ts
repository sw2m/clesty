/**
 * Red-Gate scaffold for sw2m/clesty issue #250 — `unevaluatedProperties`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #250 grows a tech
...
// grows the wiring for `unevaluatedProperties`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/unevaluatedproperties/", import.meta.url),
);
...
Deno.test.ignore(
  "#250 (wip): `unevaluatedProperties` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #250 pins down the
...
Deno.test.ignore(
  "#250 (wip): `unevaluatedProperties` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #250 pins the
