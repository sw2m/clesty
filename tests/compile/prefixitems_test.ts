/**
 * Red-Gate scaffold for sw2m/clesty issue #248 — `prefixItems`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #248 grows a tech
...
// grows the wiring for `prefixItems`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/prefixitems/", import.meta.url),
);
...
Deno.test.ignore(
  "#248 (wip): `prefixItems` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #248 pins down the
...
Deno.test.ignore(
  "#248 (wip): `prefixItems` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #248 pins the
