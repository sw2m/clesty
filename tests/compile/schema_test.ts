/**
 * Red-Gate scaffold for sw2m/clesty issue #258 — `$schema`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #258 grows a tech
...
Deno.test.ignore(
  "#258 (wip): `$schema` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #258 pins down the
...
Deno.test.ignore(
  "#258 (wip): `$schema` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #258 pins the
