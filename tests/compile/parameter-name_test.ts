/**
 * Red-Gate scaffold for sw2m/clesty issue #160 — Parameter `name`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #160 grows a tech
...
// grows the wiring for Parameter `name`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-name/", import.meta.url),
);
...
Deno.test.ignore(
  "#160 (wip): Parameter `name` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #160 pins down the
...
Deno.test.ignore(
  "#160 (wip): Parameter `name` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #160 pins the
