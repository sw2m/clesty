/**
 * Red-Gate scaffold for sw2m/clesty issue #256 — `$dynamicAnchor` / `$dynamicRef`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #256 grows a tech
...
// grows the wiring for `$dynamicAnchor` / `$dynamicRef`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/dynamicanchor-dynamicref/", import.meta.url),
);
...
Deno.test.ignore(
  "#256 (wip): `$dynamicAnchor` / `$dynamicRef` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #256 pins down the
...
Deno.test.ignore(
  "#256 (wip): `$dynamicAnchor` / `$dynamicRef` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #256 pins the
