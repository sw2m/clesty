/**
 * Red-Gate scaffold for sw2m/clesty issue #394 — `style` value invalid for `in`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #394 grows a tech
...
// grows the wiring for `style` value invalid for `in`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/style-value-invalid-for-in/", import.meta.url),
);
...
Deno.test.ignore(
  "#394 (wip): `style` value invalid for `in` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #394 pins down the
...
Deno.test.ignore(
  "#394 (wip): `style` value invalid for `in` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #394 pins the
