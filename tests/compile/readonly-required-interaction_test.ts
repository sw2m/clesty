/**
 * Red-Gate scaffold for sw2m/clesty issue #231 — `readOnly` + `required` interaction.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #231 grows a tech
...
// grows the wiring for `readOnly` + `required` interaction.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/readonly-required-interaction/", import.meta.url),
);
...
Deno.test.ignore(
  "#231 (wip): `readOnly` + `required` interaction — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #231 pins down the
...
Deno.test.ignore(
  "#231 (wip): `readOnly` + `required` interaction — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #231 pins the
