/**
 * Red-Gate scaffold for sw2m/clesty issue #428 — `required` property listed but not declared in `properties`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #428 grows a tech
...
// grows the wiring for `required` property listed but not declared in `properties`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/required-property-listed-but-not-declared-in-properties/", import.meta.url),
);
...
Deno.test.ignore(
  "#428 (wip): `required` property listed but not declared in `properties` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #428 pins down the
...
Deno.test.ignore(
  "#428 (wip): `required` property listed but not declared in `properties` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #428 pins the
