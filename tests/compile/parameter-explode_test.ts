/**
 * Red-Gate scaffold for sw2m/clesty issue #172 — Parameter `explode`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #172 grows a tech
...
// grows the wiring for Parameter `explode`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-explode/", import.meta.url),
);
...
Deno.test.ignore(
  "#172 (wip): Parameter `explode` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #172 pins down the
...
Deno.test.ignore(
  "#172 (wip): Parameter `explode` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #172 pins the
