/**
 * Red-Gate scaffold for sw2m/clesty issue #427 — Schema combinator nesting (`oneOf` of `allOf` of `anyOf`).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #427 grows a tech
...
// grows the wiring for Schema combinator nesting (`oneOf` of `allOf` of `anyOf`).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-combinator-nesting-oneof-of-allof-of-anyof/", import.meta.url),
);
...
Deno.test.ignore(
  "#427 (wip): Schema combinator nesting (`oneOf` of `allOf` of `anyOf`) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #427 pins down the
...
Deno.test.ignore(
  "#427 (wip): Schema combinator nesting (`oneOf` of `allOf` of `anyOf`) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #427 pins the
