/**
 * Red-Gate scaffold for sw2m/clesty issue #218 — Schema `maxProperties` / `minProperties`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #218 grows a tech
...
// grows the wiring for Schema `maxProperties` / `minProperties`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-maxproperties-minproperties/", import.meta.url),
);
...
Deno.test.ignore(
  "#218 (wip): Schema `maxProperties` / `minProperties` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #218 pins down the
...
Deno.test.ignore(
  "#218 (wip): Schema `maxProperties` / `minProperties` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #218 pins the
