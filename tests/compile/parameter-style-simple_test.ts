/**
 * Red-Gate scaffold for sw2m/clesty issue #168 — Parameter `style: simple`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #168 grows a tech
...
// grows the wiring for Parameter `style: simple`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-style-simple/", import.meta.url),
);
...
Deno.test.ignore(
  "#168 (wip): Parameter `style: simple` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #168 pins down the
...
Deno.test.ignore(
  "#168 (wip): Parameter `style: simple` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #168 pins the
