/**
 * Red-Gate scaffold for sw2m/clesty issue #376 — `responses` with only `default`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #376 grows a tech
...
// grows the wiring for `responses` with only `default`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/responses-with-only-default/", import.meta.url),
);
...
Deno.test.ignore(
  "#376 (wip): `responses` with only `default` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #376 pins down the
...
Deno.test.ignore(
  "#376 (wip): `responses` with only `default` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #376 pins the
