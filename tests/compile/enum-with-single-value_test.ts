/**
 * Red-Gate scaffold for sw2m/clesty issue #436 — `enum` with single value.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #436 grows a tech
...
// grows the wiring for `enum` with single value.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/enum-with-single-value/", import.meta.url),
);
...
Deno.test.ignore(
  "#436 (wip): `enum` with single value — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #436 pins down the
...
Deno.test.ignore(
  "#436 (wip): `enum` with single value — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #436 pins the
