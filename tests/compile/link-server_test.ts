/**
 * Red-Gate scaffold for sw2m/clesty issue #324 — Link `server`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #324 grows a tech
...
// grows the wiring for Link `server`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/link-server/", import.meta.url),
);
...
Deno.test.ignore(
  "#324 (wip): Link `server` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #324 pins down the
...
Deno.test.ignore(
  "#324 (wip): Link `server` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #324 pins the
