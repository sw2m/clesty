/**
 * Red-Gate scaffold for sw2m/clesty issue #247 — `dependentRequired`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #247 grows a tech
...
// grows the wiring for `dependentRequired`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/dependentrequired/", import.meta.url),
);
...
Deno.test.ignore(
  "#247 (wip): `dependentRequired` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #247 pins down the
...
Deno.test.ignore(
  "#247 (wip): `dependentRequired` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #247 pins the
