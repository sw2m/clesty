/**
 * Red-Gate scaffold for sw2m/clesty issue #260 — `$comment`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #260 grows a tech
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/comment/", import.meta.url),
);
...
Deno.test.ignore(
  "#260 (wip): `$comment` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #260 pins down the
...
Deno.test.ignore(
  "#260 (wip): `$comment` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #260 pins the
