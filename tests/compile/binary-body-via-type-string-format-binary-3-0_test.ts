/**
 * Red-Gate scaffold for sw2m/clesty issue #192 — Binary body via `type: string, format: binary` (3.0).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #192 grows a tech
...
// grows the wiring for Binary body via `type: string, format: binary` (3.0).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/binary-body-via-type-string-format-binary-3-0/", import.meta.url),
);
...
Deno.test.ignore(
  "#192 (wip): Binary body via `type: string, format: binary` (3.0) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #192 pins down the
...
Deno.test.ignore(
  "#192 (wip): Binary body via `type: string, format: binary` (3.0) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #192 pins the
