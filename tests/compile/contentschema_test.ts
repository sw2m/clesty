/**
 * Red-Gate scaffold for sw2m/clesty issue #263 — `contentSchema`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #263 grows a tech
...
// grows the wiring for `contentSchema`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/contentschema/", import.meta.url),
);
...
Deno.test.ignore(
  "#263 (wip): `contentSchema` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #263 pins down the
...
Deno.test.ignore(
  "#263 (wip): `contentSchema` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #263 pins the
