/**
 * Red-Gate scaffold for sw2m/clesty issue #224 — Schema `anyOf`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #224 grows a tech
...
// grows the wiring for Schema `anyOf`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-anyof/", import.meta.url),
);
...
Deno.test.ignore(
  "#224 (wip): Schema `anyOf` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #224 pins down the
...
Deno.test.ignore(
  "#224 (wip): Schema `anyOf` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #224 pins the
