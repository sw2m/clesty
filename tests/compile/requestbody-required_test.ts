/**
 * Red-Gate scaffold for sw2m/clesty issue #181 — `requestBody.required`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #181 grows a tech
...
// grows the wiring for `requestBody.required`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/requestbody-required/", import.meta.url),
);
...
Deno.test.ignore(
  "#181 (wip): `requestBody.required` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #181 pins down the
...
Deno.test.ignore(
  "#181 (wip): `requestBody.required` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #181 pins the
