/**
 * Red-Gate scaffold for sw2m/clesty issue #401 — OAuth `refreshUrl` absent.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #401 grows a tech
...
// grows the wiring for OAuth `refreshUrl` absent.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/oauth-refreshurl-absent/", import.meta.url),
);
...
Deno.test.ignore(
  "#401 (wip): OAuth `refreshUrl` absent — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #401 pins down the
...
Deno.test.ignore(
  "#401 (wip): OAuth `refreshUrl` absent — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #401 pins the
