/**
 * Red-Gate scaffold for sw2m/clesty issue #195 — Binary body via `contentEncoding` (3.1).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #195 grows a tech
...
// grows the wiring for Binary body via `contentEncoding` (3.1).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/binary-body-via-contentencoding-3-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#195 (wip): Binary body via `contentEncoding` (3.1) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #195 pins down the
...
Deno.test.ignore(
  "#195 (wip): Binary body via `contentEncoding` (3.1) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #195 pins the
