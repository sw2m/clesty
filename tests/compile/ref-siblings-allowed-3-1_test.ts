/**
 * Red-Gate scaffold for sw2m/clesty issue #266 — `$ref` siblings allowed (3.1).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #266 grows a tech
...
// grows the wiring for `$ref` siblings allowed (3.1).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ref-siblings-allowed-3-1/", import.meta.url),
);
...
Deno.test.ignore(
  "#266 (wip): `$ref` siblings allowed (3.1) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #266 pins down the
...
Deno.test.ignore(
  "#266 (wip): `$ref` siblings allowed (3.1) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #266 pins the
