/**
 * Red-Gate scaffold for sw2m/clesty issue #372 — Document with neither `paths`, `webhooks`, nor `components`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #372 grows a tech
...
// grows the wiring for Document with neither `paths`, `webhooks`, nor `components`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/document-with-neither-paths-webhooks-nor-components/", import.meta.url),
);
...
Deno.test.ignore(
  "#372 (wip): Document with neither `paths`, `webhooks`, nor `components` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #372 pins down the
...
Deno.test.ignore(
  "#372 (wip): Document with neither `paths`, `webhooks`, nor `components` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #372 pins the
