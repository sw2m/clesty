/**
 * Red-Gate scaffold for sw2m/clesty issue #358 — Mixed local + external `$ref`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #358 grows a tech
...
// grows the wiring for Mixed local + external `$ref`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/mixed-local-external-ref/", import.meta.url),
);
...
Deno.test.ignore(
  "#358 (wip): Mixed local + external `$ref` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #358 pins down the
...
Deno.test.ignore(
  "#358 (wip): Mixed local + external `$ref` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #358 pins the
