/**
 * Red-Gate scaffold for sw2m/clesty issue #188 — Encoding `explode`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #188 grows a tech
...
// grows the wiring for Encoding `explode`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/encoding-explode/", import.meta.url),
);
...
Deno.test.ignore(
  "#188 (wip): Encoding `explode` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #188 pins down the
...
Deno.test.ignore(
  "#188 (wip): Encoding `explode` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #188 pins the
