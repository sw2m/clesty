/**
 * Red-Gate scaffold for sw2m/clesty issue #362 — Parameter name collision across `in`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #362 grows a tech
...
// grows the wiring for Parameter name collision across `in`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-name-collision-across-in/", import.meta.url),
);
...
Deno.test.ignore(
  "#362 (wip): Parameter name collision across `in` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #362 pins down the
...
Deno.test.ignore(
  "#362 (wip): Parameter name collision across `in` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #362 pins the
