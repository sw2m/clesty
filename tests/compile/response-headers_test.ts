/**
 * Red-Gate scaffold for sw2m/clesty issue #201 — Response `headers`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #201 grows a tech
...
// grows the wiring for Response `headers`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-headers/", import.meta.url),
);
...
Deno.test.ignore(
  "#201 (wip): Response `headers` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #201 pins down the
...
Deno.test.ignore(
  "#201 (wip): Response `headers` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #201 pins the
