/**
 * Red-Gate scaffold for sw2m/clesty issue #243 — `const`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #243 grows a tech
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/const/", import.meta.url),
);
...
Deno.test.ignore(
  "#243 (wip): `const` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #243 pins down the
...
Deno.test.ignore(
  "#243 (wip): `const` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #243 pins the
