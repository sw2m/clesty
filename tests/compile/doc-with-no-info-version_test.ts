/**
 * Red-Gate scaffold for sw2m/clesty issue #425 — Doc with no `info.version`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #425 grows a tech
...
// grows the wiring for Doc with no `info.version`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/doc-with-no-info-version/", import.meta.url),
);
...
Deno.test.ignore(
  "#425 (wip): Doc with no `info.version` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #425 pins down the
...
Deno.test.ignore(
  "#425 (wip): Doc with no `info.version` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #425 pins the
