/**
 * Red-Gate scaffold for sw2m/clesty issue #271 — `components.requestBodies`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #271 grows a tech
...
// grows the wiring for `components.requestBodies`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-requestbodies/", import.meta.url),
);
...
Deno.test.ignore(
  "#271 (wip): `components.requestBodies` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #271 pins down the
...
Deno.test.ignore(
  "#271 (wip): `components.requestBodies` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #271 pins the
