/**
 * Red-Gate scaffold for sw2m/clesty issue #349 — `discriminator.mapping` — implicit.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #349 grows a tech
...
// grows the wiring for `discriminator.mapping` — implicit.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/discriminator-mapping-implicit/", import.meta.url),
);
...
Deno.test.ignore(
  "#349 (wip): `discriminator.mapping` — implicit — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #349 pins down the
...
Deno.test.ignore(
  "#349 (wip): `discriminator.mapping` — implicit — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #349 pins the
