/**
 * Red-Gate scaffold for sw2m/clesty issue #339 — Multiple media types on a body/response.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #339 grows a tech
...
// grows the wiring for Multiple media types on a body/response.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/multiple-media-types-on-a-body-response/", import.meta.url),
);
...
Deno.test.ignore(
  "#339 (wip): Multiple media types on a body/response — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #339 pins down the
...
Deno.test.ignore(
  "#339 (wip): Multiple media types on a body/response — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #339 pins the
