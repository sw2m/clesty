/**
 * Red-Gate scaffold for sw2m/clesty issue #391 — `servers` defined at all three levels with same url.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #391 grows a tech
...
// grows the wiring for `servers` defined at all three levels with same url.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/servers-defined-at-all-three-levels-with-same-url/", import.meta.url),
);
...
Deno.test.ignore(
  "#391 (wip): `servers` defined at all three levels with same url — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #391 pins down the
...
Deno.test.ignore(
  "#391 (wip): `servers` defined at all three levels with same url — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #391 pins the
