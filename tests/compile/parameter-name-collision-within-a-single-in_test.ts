/**
 * Red-Gate scaffold for sw2m/clesty issue #363 — Parameter name collision within a single `in`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #363 grows a tech
...
// grows the wiring for Parameter name collision within a single `in`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-name-collision-within-a-single-in/", import.meta.url),
);
...
Deno.test.ignore(
  "#363 (wip): Parameter name collision within a single `in` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #363 pins down the
...
Deno.test.ignore(
  "#363 (wip): Parameter name collision within a single `in` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #363 pins the
