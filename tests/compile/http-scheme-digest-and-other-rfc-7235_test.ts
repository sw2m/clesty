/**
 * Red-Gate scaffold for sw2m/clesty issue #290 — `http` scheme: `digest` and other RFC 7235.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #290 grows a tech
...
// grows the wiring for `http` scheme: `digest` and other RFC 7235.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/http-scheme-digest-and-other-rfc-7235/", import.meta.url),
);
...
Deno.test.ignore(
  "#290 (wip): `http` scheme: `digest` and other RFC 7235 — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #290 pins down the
...
Deno.test.ignore(
  "#290 (wip): `http` scheme: `digest` and other RFC 7235 — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #290 pins the
