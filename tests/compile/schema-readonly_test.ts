/**
 * Red-Gate scaffold for sw2m/clesty issue #228 — Schema `readOnly`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #228 grows a tech
...
// grows the wiring for Schema `readOnly`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-readonly/", import.meta.url),
);
...
Deno.test.ignore(
  "#228 (wip): Schema `readOnly` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #228 pins down the
...
Deno.test.ignore(
  "#228 (wip): Schema `readOnly` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #228 pins the
