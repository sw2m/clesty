/**
 * Red-Gate scaffold for sw2m/clesty issue #432 — Two `tags` with same `name`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #432 grows a tech
...
// grows the wiring for Two `tags` with same `name`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/two-tags-with-same-name/", import.meta.url),
);
...
Deno.test.ignore(
  "#432 (wip): Two `tags` with same `name` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #432 pins down the
...
Deno.test.ignore(
  "#432 (wip): Two `tags` with same `name` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #432 pins the
