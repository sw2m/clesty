/**
 * Red-Gate scaffold for sw2m/clesty issue #225 — Schema `not`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #225 grows a tech
...
// grows the wiring for Schema `not`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-not/", import.meta.url),
);
...
Deno.test.ignore(
  "#225 (wip): Schema `not` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #225 pins down the
...
Deno.test.ignore(
  "#225 (wip): Schema `not` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #225 pins the
