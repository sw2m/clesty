/**
 * Red-Gate scaffold for sw2m/clesty issue #443 — XDG Base Directory compliance.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #443 grows a tech
...
// grows the wiring for XDG Base Directory compliance.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/xdg-base-directory-compliance/", import.meta.url),
);
...
Deno.test.ignore(
  "#443 (wip): XDG Base Directory compliance — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #443 pins down the
...
Deno.test.ignore(
  "#443 (wip): XDG Base Directory compliance — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #443 pins the
