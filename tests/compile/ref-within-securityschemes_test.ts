/**
 * Red-Gate scaffold for sw2m/clesty issue #299 — `$ref` within `securitySchemes`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #299 grows a tech
...
// grows the wiring for `$ref` within `securitySchemes`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ref-within-securityschemes/", import.meta.url),
);
...
Deno.test.ignore(
  "#299 (wip): `$ref` within `securitySchemes` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #299 pins down the
...
Deno.test.ignore(
  "#299 (wip): `$ref` within `securitySchemes` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #299 pins the
