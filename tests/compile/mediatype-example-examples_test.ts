/**
 * Red-Gate scaffold for sw2m/clesty issue #183 — MediaType `example` / `examples`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #183 grows a tech
...
// grows the wiring for MediaType `example` / `examples`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/mediatype-example-examples/", import.meta.url),
);
...
Deno.test.ignore(
  "#183 (wip): MediaType `example` / `examples` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #183 pins down the
...
Deno.test.ignore(
  "#183 (wip): MediaType `example` / `examples` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #183 pins the
