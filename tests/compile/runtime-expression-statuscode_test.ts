/**
 * Red-Gate scaffold for sw2m/clesty issue #312 — Runtime expression `$statusCode`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #312 grows a tech
...
// grows the wiring for Runtime expression `$statusCode`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-statuscode/", import.meta.url),
);
...
Deno.test.ignore(
  "#312 (wip): Runtime expression `$statusCode` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #312 pins down the
...
Deno.test.ignore(
  "#312 (wip): Runtime expression `$statusCode` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #312 pins the
