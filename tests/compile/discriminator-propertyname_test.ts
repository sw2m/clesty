/**
 * Red-Gate scaffold for sw2m/clesty issue #348 — `discriminator.propertyName`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #348 grows a tech
...
// grows the wiring for `discriminator.propertyName`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/discriminator-propertyname/", import.meta.url),
);
...
Deno.test.ignore(
  "#348 (wip): `discriminator.propertyName` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #348 pins down the
...
Deno.test.ignore(
  "#348 (wip): `discriminator.propertyName` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #348 pins the
