/**
 * Red-Gate scaffold for sw2m/clesty issue #166 — Parameter `style: label`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #166 grows a tech
...
// grows the wiring for Parameter `style: label`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-style-label/", import.meta.url),
);
...
Deno.test.ignore(
  "#166 (wip): Parameter `style: label` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #166 pins down the
...
Deno.test.ignore(
  "#166 (wip): Parameter `style: label` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #166 pins the
