/**
 * Red-Gate scaffold for sw2m/clesty issue #395 — `explode` defaults differ by `style`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #395 grows a tech
...
// grows the wiring for `explode` defaults differ by `style`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/explode-defaults-differ-by-style/", import.meta.url),
);
...
Deno.test.ignore(
  "#395 (wip): `explode` defaults differ by `style` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #395 pins down the
...
Deno.test.ignore(
  "#395 (wip): `explode` defaults differ by `style` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #395 pins the
