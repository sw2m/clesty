/**
 * Red-Gate scaffold for sw2m/clesty issue #203 — Response Header `content`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #203 grows a tech
...
// grows the wiring for Response Header `content`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-header-content/", import.meta.url),
);
...
Deno.test.ignore(
  "#203 (wip): Response Header `content` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #203 pins down the
...
Deno.test.ignore(
  "#203 (wip): Response Header `content` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #203 pins the
