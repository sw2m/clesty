/**
 * Red-Gate scaffold for sw2m/clesty issue #206 — Response `content`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #206 grows a tech
...
// grows the wiring for Response `content`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-content/", import.meta.url),
);
...
Deno.test.ignore(
  "#206 (wip): Response `content` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #206 pins down the
...
Deno.test.ignore(
  "#206 (wip): Response `content` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #206 pins the
