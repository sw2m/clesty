/**
 * Red-Gate scaffold for sw2m/clesty issue #327 — Webhook entry as Path Item.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #327 grows a tech
...
// grows the wiring for Webhook entry as Path Item.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/webhook-entry-as-path-item/", import.meta.url),
);
...
Deno.test.ignore(
  "#327 (wip): Webhook entry as Path Item — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #327 pins down the
...
Deno.test.ignore(
  "#327 (wip): Webhook entry as Path Item — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #327 pins the
