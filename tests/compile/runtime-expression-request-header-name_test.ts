/**
 * Red-Gate scaffold for sw2m/clesty issue #315 — Runtime expression `$request.header.<name>`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #315 grows a tech
...
// grows the wiring for Runtime expression `$request.header.<name>`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-request-header-name/", import.meta.url),
);
...
Deno.test.ignore(
  "#315 (wip): Runtime expression `$request.header.<name>` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #315 pins down the
...
Deno.test.ignore(
  "#315 (wip): Runtime expression `$request.header.<name>` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #315 pins the
