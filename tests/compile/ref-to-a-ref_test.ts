/**
 * Red-Gate scaffold for sw2m/clesty issue #356 — `$ref` to a `$ref`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #356 grows a tech
...
// grows the wiring for `$ref` to a `$ref`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ref-to-a-ref/", import.meta.url),
);
...
Deno.test.ignore(
  "#356 (wip): `$ref` to a `$ref` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #356 pins down the
...
Deno.test.ignore(
  "#356 (wip): `$ref` to a `$ref` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #356 pins the
