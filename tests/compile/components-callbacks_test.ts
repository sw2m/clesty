/**
 * Red-Gate scaffold for sw2m/clesty issue #275 — `components.callbacks`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #275 grows a tech
...
// grows the wiring for `components.callbacks`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-callbacks/", import.meta.url),
);
...
Deno.test.ignore(
  "#275 (wip): `components.callbacks` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #275 pins down the
...
Deno.test.ignore(
  "#275 (wip): `components.callbacks` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #275 pins the
