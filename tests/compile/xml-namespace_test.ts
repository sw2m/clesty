/**
 * Red-Gate scaffold for sw2m/clesty issue #341 — `xml.namespace`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #341 grows a tech
...
// grows the wiring for `xml.namespace`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/xml-namespace/", import.meta.url),
);
...
Deno.test.ignore(
  "#341 (wip): `xml.namespace` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #341 pins down the
...
Deno.test.ignore(
  "#341 (wip): `xml.namespace` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #341 pins the
