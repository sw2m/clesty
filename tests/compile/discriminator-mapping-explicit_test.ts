/**
 * Red-Gate scaffold for sw2m/clesty issue #350 — `discriminator.mapping` — explicit.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #350 grows a tech
...
// grows the wiring for `discriminator.mapping` — explicit.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/discriminator-mapping-explicit/", import.meta.url),
);
...
Deno.test.ignore(
  "#350 (wip): `discriminator.mapping` — explicit — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #350 pins down the
...
Deno.test.ignore(
  "#350 (wip): `discriminator.mapping` — explicit — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #350 pins the
