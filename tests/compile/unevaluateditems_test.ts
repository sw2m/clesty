/**
 * Red-Gate scaffold for sw2m/clesty issue #251 — `unevaluatedItems`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #251 grows a tech
...
// grows the wiring for `unevaluatedItems`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/unevaluateditems/", import.meta.url),
);
...
Deno.test.ignore(
  "#251 (wip): `unevaluatedItems` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #251 pins down the
...
Deno.test.ignore(
  "#251 (wip): `unevaluatedItems` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #251 pins the
