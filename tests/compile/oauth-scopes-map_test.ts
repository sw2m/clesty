/**
 * Red-Gate scaffold for sw2m/clesty issue #295 — OAuth `scopes` map.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #295 grows a tech
...
// grows the wiring for OAuth `scopes` map.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/oauth-scopes-map/", import.meta.url),
);
...
Deno.test.ignore(
  "#295 (wip): OAuth `scopes` map — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #295 pins down the
...
Deno.test.ignore(
  "#295 (wip): OAuth `scopes` map — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #295 pins the
