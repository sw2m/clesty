/**
 * Red-Gate scaffold for sw2m/clesty issue #353 — Discriminator with `allOf` (parent/child).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #353 grows a tech
...
// grows the wiring for Discriminator with `allOf` (parent/child).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/discriminator-with-allof-parent-child/", import.meta.url),
);
...
Deno.test.ignore(
  "#353 (wip): Discriminator with `allOf` (parent/child) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #353 pins down the
...
Deno.test.ignore(
  "#353 (wip): Discriminator with `allOf` (parent/child) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #353 pins the
