/**
 * Red-Gate scaffold for sw2m/clesty issue #337 — Media type wildcard subtype `image/*`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #337 grows a tech
...
// grows the wiring for Media type wildcard subtype `image/*`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/media-type-wildcard-subtype-image/", import.meta.url),
);
...
Deno.test.ignore(
  "#337 (wip): Media type wildcard subtype `image/*` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #337 pins down the
...
Deno.test.ignore(
  "#337 (wip): Media type wildcard subtype `image/*` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #337 pins the
