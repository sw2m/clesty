/**
 * Red-Gate scaffold for sw2m/clesty issue #178 — Parameter `schema` vs `content` mutual exclusion.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #178 grows a tech
...
// grows the wiring for Parameter `schema` vs `content` mutual exclusion.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-schema-vs-content-mutual-exclusion/", import.meta.url),
);
...
Deno.test.ignore(
  "#178 (wip): Parameter `schema` vs `content` mutual exclusion — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #178 pins down the
...
Deno.test.ignore(
  "#178 (wip): Parameter `schema` vs `content` mutual exclusion — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #178 pins the
