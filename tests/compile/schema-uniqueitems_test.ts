/**
 * Red-Gate scaffold for sw2m/clesty issue #217 — Schema `uniqueItems`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #217 grows a tech
...
// grows the wiring for Schema `uniqueItems`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-uniqueitems/", import.meta.url),
);
...
Deno.test.ignore(
  "#217 (wip): Schema `uniqueItems` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #217 pins down the
...
Deno.test.ignore(
  "#217 (wip): Schema `uniqueItems` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #217 pins the
