/**
 * Red-Gate scaffold for sw2m/clesty issue #329 — `example` (singular) on Schema/Parameter/Header/MediaType.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #329 grows a tech
...
// grows the wiring for `example` (singular) on Schema/Parameter/Header/MediaType.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/example-singular-on-schema-parameter-header-mediatype/", import.meta.url),
);
...
Deno.test.ignore(
  "#329 (wip): `example` (singular) on Schema/Parameter/Header/MediaType — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #329 pins down the
...
Deno.test.ignore(
  "#329 (wip): `example` (singular) on Schema/Parameter/Header/MediaType — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #329 pins the
