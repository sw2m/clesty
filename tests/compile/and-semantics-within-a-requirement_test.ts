/**
 * Red-Gate scaffold for sw2m/clesty issue #302 — AND semantics within a requirement.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #302 grows a tech
...
// grows the wiring for AND semantics within a requirement.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/and-semantics-within-a-requirement/", import.meta.url),
);
...
Deno.test.ignore(
  "#302 (wip): AND semantics within a requirement — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #302 pins down the
...
Deno.test.ignore(
  "#302 (wip): AND semantics within a requirement — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #302 pins the
