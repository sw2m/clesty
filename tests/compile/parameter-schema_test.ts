/**
 * Red-Gate scaffold for sw2m/clesty issue #174 — Parameter `schema`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #174 grows a tech
...
// grows the wiring for Parameter `schema`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-schema/", import.meta.url),
);
...
Deno.test.ignore(
  "#174 (wip): Parameter `schema` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #174 pins down the
...
Deno.test.ignore(
  "#174 (wip): Parameter `schema` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #174 pins the
