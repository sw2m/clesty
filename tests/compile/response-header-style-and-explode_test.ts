/**
 * Red-Gate scaffold for sw2m/clesty issue #205 — Response Header `style` and `explode`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #205 grows a tech
...
// grows the wiring for Response Header `style` and `explode`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-header-style-and-explode/", import.meta.url),
);
...
Deno.test.ignore(
  "#205 (wip): Response Header `style` and `explode` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #205 pins down the
...
Deno.test.ignore(
  "#205 (wip): Response Header `style` and `explode` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #205 pins the
