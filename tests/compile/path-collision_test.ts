/**
 * Red-Gate scaffold for sw2m/clesty issue #366 — Path collision.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #366 grows a tech
...
// grows the wiring for Path collision.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/path-collision/", import.meta.url),
);
...
Deno.test.ignore(
  "#366 (wip): Path collision — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #366 pins down the
...
Deno.test.ignore(
  "#366 (wip): Path collision — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #366 pins the
