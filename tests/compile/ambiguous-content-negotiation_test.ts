/**
 * Red-Gate scaffold for sw2m/clesty issue #378 — Ambiguous content negotiation.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #378 grows a tech
...
// grows the wiring for Ambiguous content negotiation.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/ambiguous-content-negotiation/", import.meta.url),
);
...
Deno.test.ignore(
  "#378 (wip): Ambiguous content negotiation — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #378 pins down the
...
Deno.test.ignore(
  "#378 (wip): Ambiguous content negotiation — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #378 pins the
