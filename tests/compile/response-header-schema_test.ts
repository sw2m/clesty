/**
 * Red-Gate scaffold for sw2m/clesty issue #202 — Response Header `schema`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #202 grows a tech
...
// grows the wiring for Response Header `schema`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-header-schema/", import.meta.url),
);
...
Deno.test.ignore(
  "#202 (wip): Response Header `schema` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #202 pins down the
...
Deno.test.ignore(
  "#202 (wip): Response Header `schema` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #202 pins the
