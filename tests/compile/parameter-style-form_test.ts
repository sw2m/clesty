/**
 * Red-Gate scaffold for sw2m/clesty issue #167 — Parameter `style: form`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #167 grows a tech
...
// grows the wiring for Parameter `style: form`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-style-form/", import.meta.url),
);
...
Deno.test.ignore(
  "#167 (wip): Parameter `style: form` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #167 pins down the
...
Deno.test.ignore(
  "#167 (wip): Parameter `style: form` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #167 pins the
