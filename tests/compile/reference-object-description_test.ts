/**
 * Red-Gate scaffold for sw2m/clesty issue #282 — Reference Object `description`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #282 grows a tech
...
// grows the wiring for Reference Object `description`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/reference-object-description/", import.meta.url),
);
...
Deno.test.ignore(
  "#282 (wip): Reference Object `description` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #282 pins down the
...
Deno.test.ignore(
  "#282 (wip): Reference Object `description` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #282 pins the
