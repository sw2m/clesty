/**
 * Red-Gate scaffold for sw2m/clesty issue #211 — Schema `multipleOf`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #211 grows a tech
...
// grows the wiring for Schema `multipleOf`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-multipleof/", import.meta.url),
);
...
Deno.test.ignore(
  "#211 (wip): Schema `multipleOf` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #211 pins down the
...
Deno.test.ignore(
  "#211 (wip): Schema `multipleOf` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #211 pins the
