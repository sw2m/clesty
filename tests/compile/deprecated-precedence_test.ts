/**
 * Red-Gate scaffold for sw2m/clesty issue #423 — `deprecated` precedence.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #423 grows a tech
...
// grows the wiring for `deprecated` precedence.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/deprecated-precedence/", import.meta.url),
);
...
Deno.test.ignore(
  "#423 (wip): `deprecated` precedence — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #423 pins down the
...
Deno.test.ignore(
  "#423 (wip): `deprecated` precedence — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #423 pins the
