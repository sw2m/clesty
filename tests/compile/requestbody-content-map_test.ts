/**
 * Red-Gate scaffold for sw2m/clesty issue #180 — `requestBody.content` (map).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #180 grows a tech
...
// grows the wiring for `requestBody.content` (map).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/requestbody-content-map/", import.meta.url),
);
...
Deno.test.ignore(
  "#180 (wip): `requestBody.content` (map) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #180 pins down the
...
Deno.test.ignore(
  "#180 (wip): `requestBody.content` (map) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #180 pins the
