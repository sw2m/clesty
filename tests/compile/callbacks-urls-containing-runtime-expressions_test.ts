/**
 * Red-Gate scaffold for sw2m/clesty issue #405 — `callbacks` URLs containing runtime expressions.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #405 grows a tech
...
// grows the wiring for `callbacks` URLs containing runtime expressions.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/callbacks-urls-containing-runtime-expressions/", import.meta.url),
);
...
Deno.test.ignore(
  "#405 (wip): `callbacks` URLs containing runtime expressions — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #405 pins down the
...
Deno.test.ignore(
  "#405 (wip): `callbacks` URLs containing runtime expressions — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #405 pins the
