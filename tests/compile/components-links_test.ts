/**
 * Red-Gate scaffold for sw2m/clesty issue #274 — `components.links`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #274 grows a tech
...
// grows the wiring for `components.links`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-links/", import.meta.url),
);
...
Deno.test.ignore(
  "#274 (wip): `components.links` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #274 pins down the
...
Deno.test.ignore(
  "#274 (wip): `components.links` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #274 pins the
