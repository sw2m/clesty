/**
 * Red-Gate scaffold for sw2m/clesty issue #404 — `callbacks` referencing parameters of parent operation.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #404 grows a tech
...
// grows the wiring for `callbacks` referencing parameters of parent operation.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/callbacks-referencing-parameters-of-parent-operation/", import.meta.url),
);
...
Deno.test.ignore(
  "#404 (wip): `callbacks` referencing parameters of parent operation — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #404 pins down the
...
Deno.test.ignore(
  "#404 (wip): `callbacks` referencing parameters of parent operation — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #404 pins the
