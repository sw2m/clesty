/**
 * Red-Gate scaffold for sw2m/clesty issue #254 — `$id`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #254 grows a tech
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/id/", import.meta.url),
);
...
Deno.test.ignore(
  "#254 (wip): `$id` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #254 pins down the
...
Deno.test.ignore(
  "#254 (wip): `$id` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #254 pins the
