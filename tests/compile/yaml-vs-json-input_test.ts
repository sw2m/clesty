/**
 * Red-Gate scaffold for sw2m/clesty issue #418 — YAML vs JSON input.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #418 grows a tech
...
// grows the wiring for YAML vs JSON input.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/yaml-vs-json-input/", import.meta.url),
);
...
Deno.test.ignore(
  "#418 (wip): YAML vs JSON input — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #418 pins down the
...
Deno.test.ignore(
  "#418 (wip): YAML vs JSON input — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #418 pins the
