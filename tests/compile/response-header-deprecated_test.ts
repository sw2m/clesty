/**
 * Red-Gate scaffold for sw2m/clesty issue #204 — Response Header `deprecated`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #204 grows a tech
...
// grows the wiring for Response Header `deprecated`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/response-header-deprecated/", import.meta.url),
);
...
Deno.test.ignore(
  "#204 (wip): Response Header `deprecated` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #204 pins down the
...
Deno.test.ignore(
  "#204 (wip): Response Header `deprecated` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #204 pins the
