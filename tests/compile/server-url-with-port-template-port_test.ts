/**
 * Red-Gate scaffold for sw2m/clesty issue #370 — Server URL with port template (`:{port}`).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #370 grows a tech
...
// grows the wiring for Server URL with port template (`:{port}`).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/server-url-with-port-template-port/", import.meta.url),
);
...
Deno.test.ignore(
  "#370 (wip): Server URL with port template (`:{port}`) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #370 pins down the
...
Deno.test.ignore(
  "#370 (wip): Server URL with port template (`:{port}`) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #370 pins the
