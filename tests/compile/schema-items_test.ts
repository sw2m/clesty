/**
 * Red-Gate scaffold for sw2m/clesty issue #226 — Schema `items`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #226 grows a tech
...
// grows the wiring for Schema `items`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-items/", import.meta.url),
);
...
Deno.test.ignore(
  "#226 (wip): Schema `items` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #226 pins down the
...
Deno.test.ignore(
  "#226 (wip): Schema `items` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #226 pins the
