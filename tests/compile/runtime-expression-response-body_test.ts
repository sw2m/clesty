/**
 * Red-Gate scaffold for sw2m/clesty issue #318 — Runtime expression `$response.body#/...`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #318 grows a tech
...
// grows the wiring for Runtime expression `$response.body#/...`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-response-body/", import.meta.url),
);
...
Deno.test.ignore(
  "#318 (wip): Runtime expression `$response.body#/...` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #318 pins down the
...
Deno.test.ignore(
  "#318 (wip): Runtime expression `$response.body#/...` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #318 pins the
