/**
 * Red-Gate scaffold for sw2m/clesty issue #261 — `contentMediaType`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #261 grows a tech
...
// grows the wiring for `contentMediaType`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/contentmediatype/", import.meta.url),
);
...
Deno.test.ignore(
  "#261 (wip): `contentMediaType` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #261 pins down the
...
Deno.test.ignore(
  "#261 (wip): `contentMediaType` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #261 pins the
