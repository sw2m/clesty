/**
 * Red-Gate scaffold for sw2m/clesty issue #375 — `requestBody.required: false` with no body.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #375 grows a tech
...
// grows the wiring for `requestBody.required: false` with no body.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/requestbody-required-false-with-no-body/", import.meta.url),
);
...
Deno.test.ignore(
  "#375 (wip): `requestBody.required: false` with no body — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #375 pins down the
...
Deno.test.ignore(
  "#375 (wip): `requestBody.required: false` with no body — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #375 pins the
