/**
 * Red-Gate scaffold for sw2m/clesty issue #222 — Schema `allOf`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #222 grows a tech
...
// grows the wiring for Schema `allOf`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-allof/", import.meta.url),
);
...
Deno.test.ignore(
  "#222 (wip): Schema `allOf` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #222 pins down the
...
Deno.test.ignore(
  "#222 (wip): Schema `allOf` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #222 pins the
