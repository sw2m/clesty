/**
 * Red-Gate scaffold for sw2m/clesty issue #301 — Security Requirement — empty object `{}` inside array.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #301 grows a tech
...
// grows the wiring for Security Requirement — empty object `{}` inside array.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/security-requirement-empty-object-inside-array/", import.meta.url),
);
...
Deno.test.ignore(
  "#301 (wip): Security Requirement — empty object `{}` inside array — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #301 pins down the
...
Deno.test.ignore(
  "#301 (wip): Security Requirement — empty object `{}` inside array — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #301 pins the
