/**
 * Red-Gate scaffold for sw2m/clesty issue #283 — Recursive `$ref`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #283 grows a tech
...
// grows the wiring for Recursive `$ref`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/recursive-ref/", import.meta.url),
);
...
Deno.test.ignore(
  "#283 (wip): Recursive `$ref` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #283 pins down the
...
Deno.test.ignore(
  "#283 (wip): Recursive `$ref` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #283 pins the
