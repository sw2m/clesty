/**
 * Red-Gate scaffold for sw2m/clesty issue #285 — `apiKey` — `in: query`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #285 grows a tech
...
// grows the wiring for `apiKey` — `in: query`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/apikey-in-query/", import.meta.url),
);
...
Deno.test.ignore(
  "#285 (wip): `apiKey` — `in: query` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #285 pins down the
...
Deno.test.ignore(
  "#285 (wip): `apiKey` — `in: query` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #285 pins the
