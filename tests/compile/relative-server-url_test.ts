/**
 * Red-Gate scaffold for sw2m/clesty issue #308 — Relative server URL.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #308 grows a tech
...
// grows the wiring for Relative server URL.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/relative-server-url/", import.meta.url),
);
...
Deno.test.ignore(
  "#308 (wip): Relative server URL — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #308 pins down the
...
Deno.test.ignore(
  "#308 (wip): Relative server URL — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #308 pins the
