/**
 * Red-Gate scaffold for sw2m/clesty issue #361 — Circular path-item or schema references with siblings (3.1).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #361 grows a tech
...
// grows the wiring for Circular path-item or schema references with siblings (3.1).
...
Deno.test.ignore(
  "#361 (wip): Circular path-item or schema references with siblings (3.1) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #361 pins down the
...
Deno.test.ignore(
  "#361 (wip): Circular path-item or schema references with siblings (3.1) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #361 pins the
