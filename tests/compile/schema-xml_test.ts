/**
 * Red-Gate scaffold for sw2m/clesty issue #233 — Schema `xml`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #233 grows a tech
...
// grows the wiring for Schema `xml`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-xml/", import.meta.url),
);
...
Deno.test.ignore(
  "#233 (wip): Schema `xml` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #233 pins down the
...
Deno.test.ignore(
  "#233 (wip): Schema `xml` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #233 pins the
