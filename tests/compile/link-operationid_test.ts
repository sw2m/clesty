/**
 * Red-Gate scaffold for sw2m/clesty issue #320 — Link `operationId`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #320 grows a tech
...
// grows the wiring for Link `operationId`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/link-operationid/", import.meta.url),
);
...
Deno.test.ignore(
  "#320 (wip): Link `operationId` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #320 pins down the
...
Deno.test.ignore(
  "#320 (wip): Link `operationId` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #320 pins the
