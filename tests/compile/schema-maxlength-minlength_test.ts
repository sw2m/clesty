/**
 * Red-Gate scaffold for sw2m/clesty issue #214 — Schema `maxLength` / `minLength`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #214 grows a tech
...
// grows the wiring for Schema `maxLength` / `minLength`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-maxlength-minlength/", import.meta.url),
);
...
Deno.test.ignore(
  "#214 (wip): Schema `maxLength` / `minLength` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #214 pins down the
...
Deno.test.ignore(
  "#214 (wip): Schema `maxLength` / `minLength` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #214 pins the
