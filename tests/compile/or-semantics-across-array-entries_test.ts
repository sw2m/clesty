/**
 * Red-Gate scaffold for sw2m/clesty issue #303 — OR semantics across array entries.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #303 grows a tech
...
// grows the wiring for OR semantics across array entries.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/or-semantics-across-array-entries/", import.meta.url),
);
...
Deno.test.ignore(
  "#303 (wip): OR semantics across array entries — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #303 pins down the
...
Deno.test.ignore(
  "#303 (wip): OR semantics across array entries — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #303 pins the
