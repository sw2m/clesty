/**
 * Red-Gate scaffold for sw2m/clesty issue #446 — Idempotent regeneration.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #446 grows a tech
...
// grows the wiring for Idempotent regeneration.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/idempotent-regeneration/", import.meta.url),
);
...
Deno.test.ignore(
  "#446 (wip): Idempotent regeneration — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #446 pins down the
...
Deno.test.ignore(
  "#446 (wip): Idempotent regeneration — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #446 pins the
