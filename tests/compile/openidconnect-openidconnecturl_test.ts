/**
 * Red-Gate scaffold for sw2m/clesty issue #297 — `openIdConnect` — `openIdConnectUrl`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #297 grows a tech
...
// grows the wiring for `openIdConnect` — `openIdConnectUrl`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/openidconnect-openidconnecturl/", import.meta.url),
);
...
Deno.test.ignore(
  "#297 (wip): `openIdConnect` — `openIdConnectUrl` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #297 pins down the
...
Deno.test.ignore(
  "#297 (wip): `openIdConnect` — `openIdConnectUrl` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #297 pins the
