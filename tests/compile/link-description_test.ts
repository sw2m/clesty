/**
 * Red-Gate scaffold for sw2m/clesty issue #323 — Link `description`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #323 grows a tech
...
// grows the wiring for Link `description`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/link-description/", import.meta.url),
);
...
Deno.test.ignore(
  "#323 (wip): Link `description` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #323 pins down the
...
Deno.test.ignore(
  "#323 (wip): Link `description` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #323 pins the
