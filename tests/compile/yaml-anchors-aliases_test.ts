/**
 * Red-Gate scaffold for sw2m/clesty issue #419 — YAML anchors / aliases.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #419 grows a tech
...
// grows the wiring for YAML anchors / aliases.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/yaml-anchors-aliases/", import.meta.url),
);
...
Deno.test.ignore(
  "#419 (wip): YAML anchors / aliases — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #419 pins down the
...
Deno.test.ignore(
  "#419 (wip): YAML anchors / aliases — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #419 pins the
