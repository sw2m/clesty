/**
 * Red-Gate scaffold for sw2m/clesty issue #298 — `mutualTLS` security scheme.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #298 grows a tech
...
// grows the wiring for `mutualTLS` security scheme.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/mutualtls-security-scheme/", import.meta.url),
);
...
Deno.test.ignore(
  "#298 (wip): `mutualTLS` security scheme — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #298 pins down the
...
Deno.test.ignore(
  "#298 (wip): `mutualTLS` security scheme — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #298 pins the
