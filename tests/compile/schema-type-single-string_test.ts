/**
 * Red-Gate scaffold for sw2m/clesty issue #208 — Schema `type` (single string).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #208 grows a tech
...
// grows the wiring for Schema `type` (single string).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-type-single-string/", import.meta.url),
);
...
Deno.test.ignore(
  "#208 (wip): Schema `type` (single string) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #208 pins down the
...
Deno.test.ignore(
  "#208 (wip): Schema `type` (single string) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #208 pins the
