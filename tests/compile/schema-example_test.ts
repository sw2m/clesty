/**
 * Red-Gate scaffold for sw2m/clesty issue #235 — Schema `example`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #235 grows a tech
...
// grows the wiring for Schema `example`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-example/", import.meta.url),
);
...
Deno.test.ignore(
  "#235 (wip): Schema `example` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #235 pins down the
...
Deno.test.ignore(
  "#235 (wip): Schema `example` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #235 pins the
