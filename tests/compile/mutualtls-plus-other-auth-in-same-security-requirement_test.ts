/**
 * Red-Gate scaffold for sw2m/clesty issue #403 — `mutualTLS` plus other auth in same security requirement.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #403 grows a tech
...
// grows the wiring for `mutualTLS` plus other auth in same security requirement.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/mutualtls-plus-other-auth-in-same-security-requirement/", import.meta.url),
);
...
Deno.test.ignore(
  "#403 (wip): `mutualTLS` plus other auth in same security requirement — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #403 pins down the
...
Deno.test.ignore(
  "#403 (wip): `mutualTLS` plus other auth in same security requirement — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #403 pins the
