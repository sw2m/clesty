/**
 * Red-Gate scaffold for sw2m/clesty issue #381 — Deeply nested schema (50+ levels).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #381 grows a tech
...
// grows the wiring for Deeply nested schema (50+ levels).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/deeply-nested-schema-50-levels/", import.meta.url),
);
...
Deno.test.ignore(
  "#381 (wip): Deeply nested schema (50+ levels) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #381 pins down the
...
Deno.test.ignore(
  "#381 (wip): Deeply nested schema (50+ levels) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #381 pins the
