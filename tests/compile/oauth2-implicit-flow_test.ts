/**
 * Red-Gate scaffold for sw2m/clesty issue #291 — `oauth2.implicit` flow.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #291 grows a tech
...
// grows the wiring for `oauth2.implicit` flow.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/oauth2-implicit-flow/", import.meta.url),
);
...
Deno.test.ignore(
  "#291 (wip): `oauth2.implicit` flow — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #291 pins down the
...
Deno.test.ignore(
  "#291 (wip): `oauth2.implicit` flow — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #291 pins the
