/**
 * Red-Gate scaffold for sw2m/clesty issue #352 — Discriminator with `anyOf`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #352 grows a tech
...
// grows the wiring for Discriminator with `anyOf`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/discriminator-with-anyof/", import.meta.url),
);
...
Deno.test.ignore(
  "#352 (wip): Discriminator with `anyOf` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #352 pins down the
...
Deno.test.ignore(
  "#352 (wip): Discriminator with `anyOf` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #352 pins the
