/**
 * Red-Gate scaffold for sw2m/clesty issue #184 — MediaType `encoding` map.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #184 grows a tech
...
// grows the wiring for MediaType `encoding` map.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/mediatype-encoding-map/", import.meta.url),
);
...
Deno.test.ignore(
  "#184 (wip): MediaType `encoding` map — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #184 pins down the
...
Deno.test.ignore(
  "#184 (wip): MediaType `encoding` map — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #184 pins the
