/**
 * Red-Gate scaffold for sw2m/clesty issue #236 — Schema `deprecated`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #236 grows a tech
...
// grows the wiring for Schema `deprecated`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-deprecated/", import.meta.url),
);
...
Deno.test.ignore(
  "#236 (wip): Schema `deprecated` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #236 pins down the
...
Deno.test.ignore(
  "#236 (wip): Schema `deprecated` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #236 pins the
