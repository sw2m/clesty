/**
 * Red-Gate scaffold for sw2m/clesty issue #158 — Parameter `in: path`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #158 grows a tech
...
// grows the wiring for Parameter `in: path`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-in-path/", import.meta.url),
);
...
Deno.test.ignore(
  "#158 (wip): Parameter `in: path` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #158 pins down the
...
Deno.test.ignore(
  "#158 (wip): Parameter `in: path` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #158 pins the
