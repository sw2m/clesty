/**
 * Red-Gate scaffold for sw2m/clesty issue #239 — Schema `title`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #239 grows a tech
...
// grows the wiring for Schema `title`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-title/", import.meta.url),
);
...
Deno.test.ignore(
  "#239 (wip): Schema `title` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #239 pins down the
...
Deno.test.ignore(
  "#239 (wip): Schema `title` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #239 pins the
