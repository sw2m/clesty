/**
 * Red-Gate scaffold for sw2m/clesty issue #164 — Parameter `allowEmptyValue`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #164 grows a tech
...
// grows the wiring for Parameter `allowEmptyValue`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-allowemptyvalue/", import.meta.url),
);
...
Deno.test.ignore(
  "#164 (wip): Parameter `allowEmptyValue` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #164 pins down the
...
Deno.test.ignore(
  "#164 (wip): Parameter `allowEmptyValue` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #164 pins the
