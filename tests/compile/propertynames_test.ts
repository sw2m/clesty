/**
 * Red-Gate scaffold for sw2m/clesty issue #252 — `propertyNames`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #252 grows a tech
...
// grows the wiring for `propertyNames`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/propertynames/", import.meta.url),
);
...
Deno.test.ignore(
  "#252 (wip): `propertyNames` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #252 pins down the
...
Deno.test.ignore(
  "#252 (wip): `propertyNames` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #252 pins the
