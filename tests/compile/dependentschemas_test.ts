/**
 * Red-Gate scaffold for sw2m/clesty issue #246 — `dependentSchemas`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #246 grows a tech
...
// grows the wiring for `dependentSchemas`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/dependentschemas/", import.meta.url),
);
...
Deno.test.ignore(
  "#246 (wip): `dependentSchemas` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #246 pins down the
...
Deno.test.ignore(
  "#246 (wip): `dependentSchemas` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #246 pins the
