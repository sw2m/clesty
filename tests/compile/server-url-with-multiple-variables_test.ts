/**
 * Red-Gate scaffold for sw2m/clesty issue #306 — Server URL with multiple variables.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #306 grows a tech
...
// grows the wiring for Server URL with multiple variables.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/server-url-with-multiple-variables/", import.meta.url),
);
...
Deno.test.ignore(
  "#306 (wip): Server URL with multiple variables — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #306 pins down the
...
Deno.test.ignore(
  "#306 (wip): Server URL with multiple variables — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #306 pins the
