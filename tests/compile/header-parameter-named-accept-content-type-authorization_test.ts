/**
 * Red-Gate scaffold for sw2m/clesty issue #392 — Header parameter named `Accept`/`Content-Type`/`Authorization`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #392 grows a tech
...
// grows the wiring for Header parameter named `Accept`/`Content-Type`/`Authorization`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/header-parameter-named-accept-content-type-authorization/", import.meta.url),
);
...
Deno.test.ignore(
  "#392 (wip): Header parameter named `Accept`/`Content-Type`/`Authorization` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #392 pins down the
...
Deno.test.ignore(
  "#392 (wip): Header parameter named `Accept`/`Content-Type`/`Authorization` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #392 pins the
