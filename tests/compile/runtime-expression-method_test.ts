/**
 * Red-Gate scaffold for sw2m/clesty issue #311 — Runtime expression `$method`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #311 grows a tech
...
// grows the wiring for Runtime expression `$method`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-method/", import.meta.url),
);
...
Deno.test.ignore(
  "#311 (wip): Runtime expression `$method` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #311 pins down the
...
Deno.test.ignore(
  "#311 (wip): Runtime expression `$method` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #311 pins the
