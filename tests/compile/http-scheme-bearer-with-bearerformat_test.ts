/**
 * Red-Gate scaffold for sw2m/clesty issue #289 — `http` scheme: `bearer` (with `bearerFormat`).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #289 grows a tech
...
// grows the wiring for `http` scheme: `bearer` (with `bearerFormat`).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/http-scheme-bearer-with-bearerformat/", import.meta.url),
);
...
Deno.test.ignore(
  "#289 (wip): `http` scheme: `bearer` (with `bearerFormat`) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #289 pins down the
...
Deno.test.ignore(
  "#289 (wip): `http` scheme: `bearer` (with `bearerFormat`) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #289 pins the
