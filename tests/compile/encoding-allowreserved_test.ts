/**
 * Red-Gate scaffold for sw2m/clesty issue #189 — Encoding `allowReserved`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #189 grows a tech
...
// grows the wiring for Encoding `allowReserved`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/encoding-allowreserved/", import.meta.url),
);
...
Deno.test.ignore(
  "#189 (wip): Encoding `allowReserved` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #189 pins down the
...
Deno.test.ignore(
  "#189 (wip): Encoding `allowReserved` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #189 pins the
