/**
 * Red-Gate scaffold for sw2m/clesty issue #317 — Runtime expression `$response.header.<name>`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #317 grows a tech
...
// grows the wiring for Runtime expression `$response.header.<name>`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-response-header-name/", import.meta.url),
);
...
Deno.test.ignore(
  "#317 (wip): Runtime expression `$response.header.<name>` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #317 pins down the
...
Deno.test.ignore(
  "#317 (wip): Runtime expression `$response.header.<name>` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #317 pins the
