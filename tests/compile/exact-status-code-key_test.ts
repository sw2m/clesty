/**
 * Red-Gate scaffold for sw2m/clesty issue #197 — Exact status code key.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #197 grows a tech
...
// grows the wiring for Exact status code key.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/exact-status-code-key/", import.meta.url),
);
...
Deno.test.ignore(
  "#197 (wip): Exact status code key — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #197 pins down the
...
Deno.test.ignore(
  "#197 (wip): Exact status code key — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #197 pins the
