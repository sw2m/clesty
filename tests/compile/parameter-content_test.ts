/**
 * Red-Gate scaffold for sw2m/clesty issue #177 — Parameter `content`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #177 grows a tech
...
// grows the wiring for Parameter `content`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-content/", import.meta.url),
);
...
Deno.test.ignore(
  "#177 (wip): Parameter `content` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #177 pins down the
...
Deno.test.ignore(
  "#177 (wip): Parameter `content` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #177 pins the
