/**
 * Red-Gate scaffold for sw2m/clesty issue #382 — Recursive types (linked-list / tree).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #382 grows a tech
...
// grows the wiring for Recursive types (linked-list / tree).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/recursive-types-linked-list-tree/", import.meta.url),
);
...
Deno.test.ignore(
  "#382 (wip): Recursive types (linked-list / tree) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #382 pins down the
...
Deno.test.ignore(
  "#382 (wip): Recursive types (linked-list / tree) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #382 pins the
