/**
 * Red-Gate scaffold for sw2m/clesty issue #374 — Mixed AND/OR in `security` array.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #374 grows a tech
...
// grows the wiring for Mixed AND/OR in `security` array.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/mixed-and-or-in-security-array/", import.meta.url),
);
...
Deno.test.ignore(
  "#374 (wip): Mixed AND/OR in `security` array — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #374 pins down the
...
Deno.test.ignore(
  "#374 (wip): Mixed AND/OR in `security` array — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #374 pins the
