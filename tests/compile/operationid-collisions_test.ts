/**
 * Red-Gate scaffold for sw2m/clesty issue #364 — `operationId` collisions.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #364 grows a tech
...
// grows the wiring for `operationId` collisions.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/operationid-collisions/", import.meta.url),
);
...
Deno.test.ignore(
  "#364 (wip): `operationId` collisions — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #364 pins down the
...
Deno.test.ignore(
  "#364 (wip): `operationId` collisions — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #364 pins the
