/**
 * Red-Gate scaffold for sw2m/clesty issue #430 — Version-mismatched references.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #430 grows a tech
...
// grows the wiring for Version-mismatched references.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/version-mismatched-references/", import.meta.url),
);
...
Deno.test.ignore(
  "#430 (wip): Version-mismatched references — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #430 pins down the
...
Deno.test.ignore(
  "#430 (wip): Version-mismatched references — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #430 pins the
