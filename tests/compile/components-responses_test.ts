/**
 * Red-Gate scaffold for sw2m/clesty issue #268 — `components.responses`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #268 grows a tech
...
// grows the wiring for `components.responses`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/components-responses/", import.meta.url),
);
...
Deno.test.ignore(
  "#268 (wip): `components.responses` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #268 pins down the
...
Deno.test.ignore(
  "#268 (wip): `components.responses` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #268 pins the
