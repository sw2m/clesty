/**
 * Red-Gate scaffold for sw2m/clesty issue #442 — Shell auto-completion generation.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #442 grows a tech
...
// grows the wiring for Shell auto-completion generation.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/shell-auto-completion-generation/", import.meta.url),
);
...
Deno.test.ignore(
  "#442 (wip): Shell auto-completion generation — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #442 pins down the
...
Deno.test.ignore(
  "#442 (wip): Shell auto-completion generation — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #442 pins the
