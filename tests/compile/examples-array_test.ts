/**
 * Red-Gate scaffold for sw2m/clesty issue #264 — `examples` (array).
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #264 grows a tech
...
// grows the wiring for `examples` (array).
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/examples-array/", import.meta.url),
);
...
Deno.test.ignore(
  "#264 (wip): `examples` (array) — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #264 pins down the
...
Deno.test.ignore(
  "#264 (wip): `examples` (array) — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #264 pins the
