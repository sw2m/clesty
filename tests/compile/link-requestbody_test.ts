/**
 * Red-Gate scaffold for sw2m/clesty issue #322 — Link `requestBody`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #322 grows a tech
...
// grows the wiring for Link `requestBody`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/link-requestbody/", import.meta.url),
);
...
Deno.test.ignore(
  "#322 (wip): Link `requestBody` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #322 pins down the
...
Deno.test.ignore(
  "#322 (wip): Link `requestBody` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #322 pins the
