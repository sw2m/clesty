/**
 * Red-Gate scaffold for sw2m/clesty issue #173 — Parameter `allowReserved`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #173 grows a tech
...
// grows the wiring for Parameter `allowReserved`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/parameter-allowreserved/", import.meta.url),
);
...
Deno.test.ignore(
  "#173 (wip): Parameter `allowReserved` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #173 pins down the
...
Deno.test.ignore(
  "#173 (wip): Parameter `allowReserved` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #173 pins the
