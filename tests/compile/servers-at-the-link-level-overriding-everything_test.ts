/**
 * Red-Gate scaffold for sw2m/clesty issue #407 — `servers` at the link level overriding everything.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #407 grows a tech
...
// grows the wiring for `servers` at the link level overriding everything.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/servers-at-the-link-level-overriding-everything/", import.meta.url),
);
...
Deno.test.ignore(
  "#407 (wip): `servers` at the link level overriding everything — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #407 pins down the
...
Deno.test.ignore(
  "#407 (wip): `servers` at the link level overriding everything — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #407 pins the
