/**
 * Red-Gate scaffold for sw2m/clesty issue #278 — External file `$ref`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #278 grows a tech
...
// grows the wiring for External file `$ref`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/external-file-ref/", import.meta.url),
);
...
Deno.test.ignore(
  "#278 (wip): External file `$ref` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #278 pins down the
...
Deno.test.ignore(
  "#278 (wip): External file `$ref` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #278 pins the
