/**
 * Red-Gate scaffold for sw2m/clesty issue #309 — Callback object as `expression → Path Item`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #309 grows a tech
...
// grows the wiring for Callback object as `expression → Path Item`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/callback-object-as-expression-path-item/", import.meta.url),
);
...
Deno.test.ignore(
  "#309 (wip): Callback object as `expression → Path Item` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #309 pins down the
...
Deno.test.ignore(
  "#309 (wip): Callback object as `expression → Path Item` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #309 pins the
