/**
 * Red-Gate scaffold for sw2m/clesty issue #365 — Missing `operationId`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #365 grows a tech
...
// grows the wiring for Missing `operationId`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/missing-operationid/", import.meta.url),
);
...
Deno.test.ignore(
  "#365 (wip): Missing `operationId` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #365 pins down the
...
Deno.test.ignore(
  "#365 (wip): Missing `operationId` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #365 pins the
