/**
 * Red-Gate scaffold for sw2m/clesty issue #244 — `exclusiveMaximum` / `exclusiveMinimum` as numbers.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #244 grows a tech
...
// grows the wiring for `exclusiveMaximum` / `exclusiveMinimum` as numbers.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/exclusivemaximum-exclusiveminimum-as-numbers/", import.meta.url),
);
...
Deno.test.ignore(
  "#244 (wip): `exclusiveMaximum` / `exclusiveMinimum` as numbers — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #244 pins down the
...
Deno.test.ignore(
  "#244 (wip): `exclusiveMaximum` / `exclusiveMinimum` as numbers — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #244 pins the
