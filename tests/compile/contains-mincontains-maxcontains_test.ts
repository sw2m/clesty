/**
 * Red-Gate scaffold for sw2m/clesty issue #249 — `contains` / `minContains` / `maxContains`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #249 grows a tech
...
// grows the wiring for `contains` / `minContains` / `maxContains`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/contains-mincontains-maxcontains/", import.meta.url),
);
...
Deno.test.ignore(
  "#249 (wip): `contains` / `minContains` / `maxContains` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #249 pins down the
...
Deno.test.ignore(
  "#249 (wip): `contains` / `minContains` / `maxContains` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #249 pins the
