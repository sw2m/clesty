/**
 * Red-Gate scaffold for sw2m/clesty issue #380 — Encoding declared for non-multipart, non-form body.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #380 grows a tech
...
// grows the wiring for Encoding declared for non-multipart, non-form body.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/encoding-declared-for-non-multipart-non-form-body/", import.meta.url),
);
...
Deno.test.ignore(
  "#380 (wip): Encoding declared for non-multipart, non-form body — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #380 pins down the
...
Deno.test.ignore(
  "#380 (wip): Encoding declared for non-multipart, non-form body — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #380 pins the
