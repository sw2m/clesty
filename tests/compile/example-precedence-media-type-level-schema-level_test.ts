/**
 * Red-Gate scaffold for sw2m/clesty issue #334 — Example precedence: media-type-level > schema-level.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #334 grows a tech
...
// grows the wiring for Example precedence: media-type-level > schema-level.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/example-precedence-media-type-level-schema-level/", import.meta.url),
);
...
Deno.test.ignore(
  "#334 (wip): Example precedence: media-type-level > schema-level — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #334 pins down the
...
Deno.test.ignore(
  "#334 (wip): Example precedence: media-type-level > schema-level — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #334 pins the
