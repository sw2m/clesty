/**
 * Red-Gate scaffold for sw2m/clesty issue #186 — Encoding `headers`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #186 grows a tech
...
// grows the wiring for Encoding `headers`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/encoding-headers/", import.meta.url),
);
...
Deno.test.ignore(
  "#186 (wip): Encoding `headers` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #186 pins down the
...
Deno.test.ignore(
  "#186 (wip): Encoding `headers` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #186 pins the
