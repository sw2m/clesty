/**
 * Red-Gate scaffold for sw2m/clesty issue #445 — Server URL trailing-slash sanitization.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #445 grows a tech
...
// grows the wiring for Server URL trailing-slash sanitization.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/server-url-trailing-slash-sanitization/", import.meta.url),
);
...
Deno.test.ignore(
  "#445 (wip): Server URL trailing-slash sanitization — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #445 pins down the
...
Deno.test.ignore(
  "#445 (wip): Server URL trailing-slash sanitization — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #445 pins the
