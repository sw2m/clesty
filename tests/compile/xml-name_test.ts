/**
 * Red-Gate scaffold for sw2m/clesty issue #340 — `xml.name`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #340 grows a tech
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/xml-name/", import.meta.url),
);
...
Deno.test.ignore(
  "#340 (wip): `xml.name` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #340 pins down the
...
Deno.test.ignore(
  "#340 (wip): `xml.name` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #340 pins the
