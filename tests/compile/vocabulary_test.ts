/**
 * Red-Gate scaffold for sw2m/clesty issue #259 — `$vocabulary`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #259 grows a tech
...
// grows the wiring for `$vocabulary`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/vocabulary/", import.meta.url),
);
...
Deno.test.ignore(
  "#259 (wip): `$vocabulary` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #259 pins down the
...
Deno.test.ignore(
  "#259 (wip): `$vocabulary` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #259 pins the
