/**
 * Red-Gate scaffold for sw2m/clesty issue #316 — Runtime expression `$request.body#/...`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #316 grows a tech
...
// grows the wiring for Runtime expression `$request.body#/...`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/runtime-expression-request-body/", import.meta.url),
);
...
Deno.test.ignore(
  "#316 (wip): Runtime expression `$request.body#/...` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #316 pins down the
...
Deno.test.ignore(
  "#316 (wip): Runtime expression `$request.body#/...` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #316 pins the
