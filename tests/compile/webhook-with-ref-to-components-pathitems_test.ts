/**
 * Red-Gate scaffold for sw2m/clesty issue #406 — Webhook with `$ref` to `components.pathItems`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #406 grows a tech
...
// grows the wiring for Webhook with `$ref` to `components.pathItems`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/webhook-with-ref-to-components-pathitems/", import.meta.url),
);
...
Deno.test.ignore(
  "#406 (wip): Webhook with `$ref` to `components.pathItems` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #406 pins down the
...
Deno.test.ignore(
  "#406 (wip): Webhook with `$ref` to `components.pathItems` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #406 pins the
