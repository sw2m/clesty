/**
 * Red-Gate scaffold for sw2m/clesty issue #232 — Schema `discriminator` reference.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #232 grows a tech
...
// grows the wiring for Schema `discriminator` reference.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-discriminator-reference/", import.meta.url),
);
...
Deno.test.ignore(
  "#232 (wip): Schema `discriminator` reference — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #232 pins down the
...
Deno.test.ignore(
  "#232 (wip): Schema `discriminator` reference — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #232 pins the
