/**
 * Red-Gate scaffold for sw2m/clesty issue #441 — Output formatter selection.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #441 grows a tech
...
// grows the wiring for Output formatter selection.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/output-formatter-selection/", import.meta.url),
);
...
Deno.test.ignore(
  "#441 (wip): Output formatter selection — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #441 pins down the
...
Deno.test.ignore(
  "#441 (wip): Output formatter selection — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #441 pins the
