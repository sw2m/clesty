/**
 * Red-Gate scaffold for sw2m/clesty issue #411 — Multipart body with mixed text and binary parts.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #411 grows a tech
...
// grows the wiring for Multipart body with mixed text and binary parts.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/multipart-body-with-mixed-text-and-binary-parts/", import.meta.url),
);
...
Deno.test.ignore(
  "#411 (wip): Multipart body with mixed text and binary parts — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #411 pins down the
...
Deno.test.ignore(
  "#411 (wip): Multipart body with mixed text and binary parts — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #411 pins the
