/**
 * Red-Gate scaffold for sw2m/clesty issue #209 — Schema `format`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #209 grows a tech
...
// grows the wiring for Schema `format`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/schema-format/", import.meta.url),
);
...
Deno.test.ignore(
  "#209 (wip): Schema `format` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #209 pins down the
...
Deno.test.ignore(
  "#209 (wip): Schema `format` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #209 pins the
