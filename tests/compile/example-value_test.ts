/**
 * Red-Gate scaffold for sw2m/clesty issue #332 — Example `value`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #332 grows a tech
...
// grows the wiring for Example `value`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/example-value/", import.meta.url),
);
...
Deno.test.ignore(
  "#332 (wip): Example `value` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #332 pins down the
...
Deno.test.ignore(
  "#332 (wip): Example `value` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #332 pins the
