/**
 * Red-Gate scaffold for sw2m/clesty issue #182 — MediaType `schema`.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #182 grows a tech
...
// grows the wiring for MediaType `schema`.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/mediatype-schema/", import.meta.url),
);
...
Deno.test.ignore(
  "#182 (wip): MediaType `schema` — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #182 pins down the
...
Deno.test.ignore(
  "#182 (wip): MediaType `schema` — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #182 pins the
