/**
 * Red-Gate scaffold for sw2m/clesty issue #393 — `allowEmptyValue` on non-query parameter.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #393 grows a tech
...
// grows the wiring for `allowEmptyValue` on non-query parameter.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/allowemptyvalue-on-non-query-parameter/", import.meta.url),
);
...
Deno.test.ignore(
  "#393 (wip): `allowEmptyValue` on non-query parameter — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #393 pins down the
...
Deno.test.ignore(
  "#393 (wip): `allowEmptyValue` on non-query parameter — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #393 pins the
