/**
 * Red-Gate scaffold for sw2m/clesty issue #431 — Op `security` empty array vs absent.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #431 grows a tech
...
// grows the wiring for Op `security` empty array vs absent.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/op-security-empty-array-vs-absent/", import.meta.url),
);
...
Deno.test.ignore(
  "#431 (wip): Op `security` empty array vs absent — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #431 pins down the
...
Deno.test.ignore(
  "#431 (wip): Op `security` empty array vs absent — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #431 pins the
