/**
 * Red-Gate scaffold for sw2m/clesty issue #347 — `x-*` placement coverage.
 *
 * staging: tests are stubbed (`Deno.test.ignore`) until #347 grows a tech
...
// grows the wiring for `x-*` placement coverage.
...
const FIXTURE_DIR = fromFileUrl(
  new URL("../fixtures/x-placement-coverage/", import.meta.url),
);
...
Deno.test.ignore(
  "#347 (wip): `x-*` placement coverage — primary contract",
  () => {
    // staging: replace this stub once the tech spec on #347 pins down the
...
Deno.test.ignore(
  "#347 (wip): `x-*` placement coverage — refusal / edge case",
  () => {
    // staging: replace this stub once the tech spec on #347 pins the
